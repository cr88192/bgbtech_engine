...
//		ent->origin=v3sub(ent->origin, vec3vf(btGetBModel(ent)->mdl_org));
		...
		ent->oldorigin=v3sub(ent->origin, ent->maxs);
	}

	//adjust for load origin
	ent->origin=v3add(ent->origin, btge_spawn_org);
...
BTSV_API void btSetFieldAsString(btEntity ent, char *var, char *val)
{
	...
	if(!strcmp(var, "origin"))
		{ ent->origin=btStrToVec3(val); return; }
...
BTSV_API void btSpawnPrefab(char *name, vec3 origin)
{
	char tb[256];
	BTGE_BrushWorld *wrl;
	vec3 oldorg;
	...
	btge_spawn_org=v3add(btge_spawn_org, origin);
	BT_SpawnWorld(wrl);
...
BTSV_API btEntity btSpawnPlayer()
{
	btEntity ent, spawn;
...
//		btParticleTeleportExplosion(vec3vf(org));
	...
	for(i=0; i<8; i++)
	{
		...
		j=btPointBoxContents(vec3vf(org1),
