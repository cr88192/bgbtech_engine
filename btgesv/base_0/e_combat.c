	...
	float damage, btEntity ignore, float radius, int mod)
{
	...
	BTGE_Voxel_UpdateBlastDamage(btge_world, origin, radius, damage);
...
//	origin=btCalcCenter(inflictor);
	...
	while(cur)
	{
		...
		dir=v3sub(org, origin);
//		pts=v3len(dir)*0.5;
		pts=btSmallDistancePointBox(origin, cur->absmin, cur->absmax);
...
BTSV_API void BT_BoxDamage(btEntity inflictor, btEntity attacker,
	float damage, btEntity ignore, vec3 mins, vec3 maxs, int mod)
{
	btEntity cur, nxt;
	float dmg;
