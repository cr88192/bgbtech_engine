...
void func_train_goStart(btEntity self)
{
	...
	self->enemy=tgt;
	self->origin=v3sub(tgt->origin, self->mins);
...
//	printf("func_train_goNext\n");
	...
	BT_CalcMove(self, v3sub(tgt->origin, self->mins), self->speed,
...
//	printf("func_train_goCurrent\n");
	...
	BT_CalcMove(self, v3sub(self->enemy->origin, self->mins), self->speed,
		func_train_movethink);
}
