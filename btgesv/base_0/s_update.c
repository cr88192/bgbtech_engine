...
//	BTGE_Inventory *ivcur;
	...
	lst=NULL;
	if((v3dist(ent->origin, ent->oldorigin)>1) ||
		(ent->flags&BT_FL_CLIENT) || fdm)
	{
		tv0[0]=v3x(ent->origin)-reforg[0];
		tv0[1]=v3y(ent->origin)-reforg[1];
		tv0[2]=v3z(ent->origin)-reforg[2];
...
//		t2=dyfloat(v3z(ent->origin));
	...
	if((v3dist(ent->origin2, ent->oldorigin2)>1) ||
		(ent->flags&BT_FL_CLIENT) || fdm)
	{
//		ent->oldorigin=ent->origin;
		t0=dyflonum(v3x(ent->origin2)-reforg[0]);
		t1=dyflonum(v3y(ent->origin2)-reforg[1]);
		t2=dyflonum(v3z(ent->origin2)-reforg[2]);
...
BTSV_API void btsvFinishUpdate()
{
	BTGE_FinishWorldUpdate();
}

BTSV_API void btsvFlushUpdate()
{
