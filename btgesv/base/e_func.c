...
//		else self->height=h;
	...
	self->pos2=v3dadd(self->origin, v3f2d(v3scale(self->movedir, h)));
...
void func_door_linkDoors(btEntity self)
{
	...
	lst=BT_CheckBoxEnts(
		v3dsub(self->absmin, vec3d(1,1,1)),
		v3dadd(self->absmax, vec3d(1,1,1)),
...
void func_door_init(btEntity self)
{
	...
	self->vieworg=v3d2f(self->origin);
...
//	self->eTrig=gcalloc(sizeof(BTGE_EntityTrigger));
	...
	self->pos1=self->origin;
	self->pos2=vec3d(999999, 999999, 999999);
	self->angles=vec3(0,0,0);
