...
{
	if(v3dx(targ->absmin)>(v3dx(self->absmax)+dist))return(false);
	if(v3dy(targ->absmin)>(v3dy(self->absmax)+dist))return(false);
	if(v3dz(targ->absmin)>(v3dz(self->absmax)+dist))return(false);
	if(v3dx(targ->absmax)<(v3dx(self->absmin)-dist))return(false);
	if(v3dy(targ->absmax)<(v3dy(self->absmin)-dist))return(false);
	if(v3dz(targ->absmax)<(v3dz(self->absmin)-dist))return(false);
	return(true);
}

BTSV_API void BT_NewGoalMove(btEntity self, btEntity targ, float dist)
{
	if(targ && (btRandom()<0.33))
	{
//		self->idealYaw=BT_TargetYaw(self, self->goalEntity);
		self->idealYaw=BT_TargetYaw(self, targ);
	}
...
BTSV_API void BT_MoveToGoal(btEntity self, float dist)
{
	...
	if(!self->goalEntity)
	{
		if(self->flags&BT_FL_AI_FREEROAM)
		{
...
//		btWalkmove(self, btCurrentYaw(self), 0);
	...
	if(!self->goalEntity && !(self->flags&BT_FL_AI_FREEROAM))
	{
		printf("BT_MoveToGoal: No GoalEntity\n");
	}

	if(self->goalEntity && (self->goalEntity==self->enemy) && 
...
//		printf("BT_MoveToGoal: %d Blocked\n", self->entnum);
		if(self->flags&BT_FL_STARTSOLID)
		{
			self->origin=v3dadd(self->origin, vec3d(0,0,10));
		}

		BT_NewGoalMove(self, self->goalEntity, dist);
		self->wanderTime=btDelayTime(4.0+btSRandom()*2.0);
...
//		printf("BT_MoveToGoal: Enemy Close Enough\n");
		...
		if(self->goalEntity)
		{
			...
			self->idealYaw=BT_TargetYaw(self, self->goalEntity);
		}else if(self->flags&BT_FL_AI_FREEROAM)
		{
			self->idealYaw=btRandom()*360;
			self->wanderTime=btDelayTime(8.0+btSRandom()*4.0);
		}
	}
}
...
BTSV_API int BT_Range(btEntity self, btEntity targ)
{
	...
	d=v3dlen(v3dsub(targ->origin, self->origin));
...
BTSV_API int BT_InfrontP(btEntity self, btEntity targ)
{
	...
	fw=BT_Forward(self);
	d=v3ddot(
		v3dnorm(v3dsub(btCalcCenter(targ), btCalcCenter(self))),
		v3f2d(fw));
...
BTSV_API float BT_EnemyYaw(btEntity self)
{
	float f;
	f=btVecToYaw(
		v3d2f(v3dsub(btCalcCenter(self->enemy),
			btCalcCenter(self))));
	return(f);
}
...
{
	float f;
	f=btVecToYaw(v3d2f(v3dsub(btCalcCenter(targ), btCalcCenter(self))));
	return(f);
}

BTSV_API vec3d BT_AimOrigin(btEntity self)
{
	return(btCalcCenter(self));
}

BTSV_API vec3 BT_AimDirection(btEntity self,
	btEntity targ, float speed)
{
	vec3d org1;
	vec3 v;
	float f, g;

	f=v3ddist(btCalcCenter(targ), btCalcCenter(self));
	g=f/speed;

	org1=v3daddscale(btCalcCenter(targ), v3f2d(targ->velocity), g);

//	v=v3norm(v3sub(targ->origin, self->origin));
//	v=v3d2f(v3dnorm(v3dsub(btCalcCenter(targ), btCalcCenter(self))));
	v=v3d2f(v3dnorm(v3dsub(org1, btCalcCenter(self))));
	return(v);
}
...
BTSV_API bool BT_FindRoamTarget(btEntity self)
{
	...
	if(self->flags&BT_FL_AI_FREEROAM)
	{
		if(self->wanderTime<btTime())
		{
			self->idealYaw=btRandom()*360;
			self->wanderTime=btDelayTime(1);
		}
		return(true);
	}
...
BTSV_API void BTAI_ChooseTurn(btEntity self, vec3 dest)
{
	...
	dir=v3sub(dest, v3d2f(self->origin));
...
BTSV_API void BTAI_Face(btEntity self)
{
	...
	dir=v3dsub(self->enemy->origin, self->origin);
	self->idealYaw=btVecToYaw(v3d2f(dir));
	BT_ChangeYaw(self);
}
