...
//	if(!self->eAct)self->eAct=gcalloc(sizeof(BTGE_EntityActor));
	...
	if(!btWalkmove(self, 0, 0))
		self->origin=v3dadd(self->origin, vec3d(0, 0, 8));
	if(!btWalkmove(self, 0, 0))
	{
		//started in solid
		btDPrint("monster %s start solid at (%s)\n",
			self->classname,
			btVec3ToStr(v3d2f(self->origin)));
	}
...
{
	self->origin=v3dadd(self->origin, vec3d(0, 0, 1));
	btDropToFloor(self);
...
void BT_MonsterMoveThink(btEntity self)
{
	...
	{
		if(!BT_CheckClientNearby(self, 8192))
		{
			if(self->flags&BT_FL_AI_DESPAWN)
			{
				btSetMoveThink(self, NULL, 0);
...
//		btSetThink(self, BT_MonsterMoveThink, 0.5);
		btSetMoveThink(self, BT_MonsterMoveThink, 0.5);
		return;
	}
		...
		(btTime()>=self->despawnTime))
	{
		...
		btSound(self, BT_CHAN_VOICE, "sound/misc/despawn0_2",
			1.0, BT_ATTN_NORM);
//		btSetThink(self, btDeleteEntity, 0.1);
		return;
	}
...
//	btSetThink(self, BT_MonsterMoveThink2, 0.1);
	btSetMoveThink(self, BT_MonsterMoveThink2, 0.1);
...
//	btSetThink(self, BT_MonsterMoveThink, 0.1);
	btSetMoveThink(self, BT_MonsterMoveThink, 0.1);
}
...
void BT_WeaponMoveThink(btEntity self)
{
	...
	move=self->weaponmove;
	if(!move)return;
	
	//check if we have reached the end
	if(self->weaponframe>=move->endFrame)
	{
		//and call final function if so
		if(move->f_end)
			move->f_end(self);
	...
	//set up think to call back here by default
//	btSetThink(self, BT_WeaponMoveThink2, 0.1);
	btSetMoveThink(self, BT_WeaponMoveThink2, 0.1);
	
	//do stuff for frame
	num=self->weaponframe-move->startFrame;
	frm=&(move->frame[num]);
	if(frm->f_ai)
		frm->f_ai(self, frm->dist);
	if(frm->f_think)
...
//		self->nextthink=-1;
	...
	//ok, set current animation in motion
	self->weaponmove=move;
	self->weaponanim=move->anim;
	self->weaponframe=move->startFrame;
//	btSetThink(self, BT_WeaponMoveThink, 0.1);
	btSetMoveThink(self, BT_WeaponMoveThink, 0.1);
}
