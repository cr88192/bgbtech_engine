	...
	BTGE_SEntity **wrl, btEntity ent)
{
	...
	if(btGetBModel(ent))
	{
		...
		BTGE_SEnt_SetStr(sent, "origin",
			btVec3ToStr(v3d2f(ent->spawnOrigin)));
...
BTSV_API BTGE_SEntity *BTGE_SaveGame_FlattenWorldBBox(
	BTGE_BrushWorld *world, vec3d amins, vec3d amaxs)
{
	BTGE_BrushWorld *wrl0;
	BTGE_SEntity *sent;
	btEntity ents, ecur;
...
//	btge_world=world;
	...
	ents=BT_CheckBoxEnts(amins, amaxs,
		BT_TRACE_BSPSOLID|BT_TRACE_MONSTERS, NULL);
	
	sent=NULL; ecur=ents;
	while(ecur)
	{
		BTGE_SaveGame_FlattenEntity(&sent, ecur);
		ecur=ecur->chain;
	}
...
byte *BTGE_SaveGame_EncodeChunkContents(BTGE_BrushWorld *world,
	BTGE_VoxelChunk *chk, byte *obuf, int osz)
{
	...
	BTGE_Voxel_BoxChunk(chk, am, an);
	sent=BTGE_SaveGame_FlattenWorldBBox(world, vec3dvf(am), vec3dvf(an));
	if(!sent)return(obuf);
	
	ct=BTGE_Voxel_FlattenRegionSEntityListBuffer(
		world, chk->rgn, sent, obuf, osz);
	return(ct);
}

byte *BTGE_SaveGame_DecodeChunkContents(BTGE_BrushWorld *world,
	BTGE_VoxelChunk *chk, byte *ibuf, int isz)
{
	return(ibuf);
}

byte *BTGE_SaveGame_EncodeRegionContents(BTGE_BrushWorld *world,
	BTGE_VoxelRegion *rgn, byte *obuf, int osz)
{
	return(obuf);
}

byte *BTGE_SaveGame_DecodeRegionContents(BTGE_BrushWorld *world,
