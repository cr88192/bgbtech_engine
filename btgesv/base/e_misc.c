...
//	self->snd_sight="sound/soldier/solsght1";
	...
	self->flags2|=BT_FL2_PUSHABLE_XY;
...
void misc_minecart_init(btEntity self)
{
	self->solidtype=BT_SOLID_BBOX;
//	self->movetype=BT_MOVE_NONE;
//	self->movetype=BT_MOVE_FLYMISSILE;
	self->movetype=BT_MOVE_SLIDE;
...
//	self->snd_sight="sound/soldier/solsght1";
	...
	self->mins=vec3(-12, -12, 8);
	self->maxs=vec3(12, 12, 32);
	self->health=180;
...
BTGE_Entity_Iface misc_minecart_vt =
{
misc_minecart_init,	info_deinit,
info_null,			info_null,
info_blocked,		info_touch,
info_use,			info_pain,
misc_minecart_die,	info_cmdmsg,
};

BTSV_API btEntity BtSp_misc_minecart(btSEntity args)
{
	btEntity self;
	self=btNewEntityTriggerInit(args, &misc_minecart_vt);	
	return(self);
}
...
void misc_waterfall_think(btEntity self)
{
	btParticles("waterglob_fall", 6, BT_RGB_WHITE,
		self->origin,
		v3scale(self->movedir, self->speed), vec3(25,25,25));
	btSetThink(self, misc_waterfall_think, 0.1);
}
...
BTSV_API btEntity BtSp_random_terrain(btSEntity args)
{
	...
	s=BTGE_SEnt_GetStr(args, "terrain_type");
	if(!s)s="default";
	btCvarSet("sv_terrain_type", s);
