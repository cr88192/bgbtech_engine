#include <btgecl.h>

// extern int btgeclui_context->fgcolor;	//text fg color
// extern int btgeclui_context->bgcolor;	//text bg color
// extern int btgeclui_context->wbgcolor;	//widget bg color

int BTGECLUI_WidgetList_Render(BTGECLUI_Widget *obj, int ox, int oy)
{
	int r, g, b;
	int i, j, k, tc;

	BTGECLUI_Forms_CalcColor(obj->bgcolor, &r, &g, &b);
	Draw_FillSquareRGBA(ox+obj->ox, oy+obj->oy,
		obj->xs, obj->ys, r, g, b, 255);

	for(i=0; i<obj->rows; i++)
	{
		j=obj->min+i;
		if(j>=obj->num_items)continue;

		tc=obj->fgcolor;
		if(j==obj->curval)
		{
			if(obj->state&BTGECLUI_GUISTATE_HASFOCUS)
				{ r=0; g=0; b=255; tc=0xFFFFFF; }
				else { r=192; g=192; b=192; }

			Draw_FillSquareRGBA(
				ox+obj->ox, oy+obj->oy+obj->ys-(i+1)*8,
				obj->xs, 8, r, g, b, 255);
		}

		GfxFont_SetFont("fixed", 0);
		BTGECLUI_Forms_CalcColor(tc, &r, &g, &b);
		GfxFont_DrawString(obj->item_value[j],
			ox+obj->ox, oy+obj->oy+obj->ys-(i+1)*8,
			8, 8, r, g, b, 255);
	}

	if(!(obj->state&65536))return(0);

	GfxFont_SetFont("gfx", 0);

	Draw_FillSquareRGBA(ox+obj->ox+obj->xs-12, oy+obj->oy,
		12, obj->ys, 192, 192, 192, 255);

	i=obj->curval*(obj->ys-36)/
		(float)obj->num_items;
	BTGECLUI_Widget_Render3DBox(ox+obj->ox+obj->xs-12,
		oy+obj->oy+obj->ys-24-i,
		12, 12, obj->wbgcolor, 0);

	BTGECLUI_Widget_Render3DBox(
		ox+obj->ox+obj->xs-12, oy+obj->oy,
		12, 12, obj->wbgcolor, 0);
	GfxFont_DrawChar(0x27,
		ox+obj->ox+obj->xs-12, oy+obj->oy,
		12, 12, 0, 0, 0, 255);

	BTGECLUI_Widget_Render3DBox(
		ox+obj->ox+obj->xs-12, oy+obj->oy+obj->ys-12,
		12, 12, obj->wbgcolor, 0);
	GfxFont_DrawChar(0x26,
		ox+obj->ox+obj->xs-12, oy+obj->oy+obj->ys-12,
		12, 12, 0, 0, 0, 255);
}

int BTGECLUI_WidgetList_HandleInput(BTGECLUI_Widget *obj,
	BTGECLUI_InputState *state, int ox, int oy)
{
	ushort *ks;
	int i;

	if(state->mb&1)obj->state&=~BTGECLUI_GUISTATE_HASFOCUS;

	if(BTGECLUI_Widget_CheckClick(ox, oy, state, obj) &&
		(obj->state&65536) && ((state->mx-ox)>=obj->xs-12))
	{
		if(!BTGECLUI_Widget_CheckClicked(ox, oy, state, obj))
			return(0);

		if(((state->my-oy)>=obj->ys-12))
		{
			obj->curval--;
			if(obj->curval<0)obj->curval=0;
			if(obj->curval<obj->min)
				obj->min=obj->curval;
		}
		if((state->my-oy)<=12)
		{
			obj->curval++;
			if(obj->curval>=obj->num_items)
				obj->curval=obj->num_items-1;
			if(obj->curval>=(obj->min+obj->rows))
				obj->min=obj->curval-obj->rows+1;
		}

		return(0);
	}

	if(BTGECLUI_Widget_CheckClick(ox, oy, state, obj))
	{
		obj->state|=BTGECLUI_GUISTATE_HASFOCUS;

		i=(state->my-oy)/8;
		i=obj->min+(obj->rows-i-1);

		if((i>=0) && (i<obj->num_items) && (obj->curval!=i))
		{
			obj->curval=i;

			BTGECLUI_Widget_AddEvent(obj, "select");
			BTGECLUI_Widget_SetEventIVal(i);
			BTGECLUI_Widget_SetEventSVal(obj->item_name[i]);
		}
	}

	if(!(obj->state&BTGECLUI_GUISTATE_HASFOCUS))
		return(0);

	i=obj->curval;

	ks=state->keys;
	while(*ks)
	{
		if(*ks==K_UPARROW)
		{
			obj->curval--;
			if(obj->curval<0)obj->curval=0;
			if(obj->curval<obj->min)
				obj->min=obj->curval;
		}
		if(*ks==K_DOWNARROW)
		{
			obj->curval++;
			if(obj->curval>=obj->num_items)
				obj->curval=obj->num_items-1;
			if(obj->curval>=(obj->min+obj->rows))
				obj->min=obj->curval-obj->rows+1;
		}

		ks++;
	}

	if((obj->curval!=i) && (obj->curval>=0))
	{
		BTGECLUI_Widget_AddEvent(obj, "select");
		BTGECLUI_Widget_SetEventIVal(obj->curval);
		BTGECLUI_Widget_SetEventSVal(
			obj->item_name[obj->curval]);
	}
}

BTGECLUI_Widget *BTGECLUI_WidgetList_Begin(char *type, char *id)
{
	BTGECLUI_Widget *tmp, *tmp2;
	char *str;
	int fg, bg, wbg;
	int i;

	tmp=gcalloc(sizeof(BTGECLUI_Widget));
	tmp->type=dystrdup(type);
	tmp->id=dystrdup(id);
	tmp->fgcolor=btgeclui_context->fgcolor;
	tmp->bgcolor=btgeclui_context->bgcolor;
	tmp->wbgcolor=btgeclui_context->wbgcolor;

	tmp->item_name=gcalloc(256*sizeof(char *));
	tmp->item_value=gcalloc(256*sizeof(char *));
	tmp->num_items=0;

	return(tmp);
}

int BTGECLUI_WidgetList_End(BTGECLUI_Widget *tmp)
{
	int i, j;

	if(!tmp->rows)
	{
		tmp->rows=tmp->num_items;
		if(tmp->rows>8)
		{
			tmp->state|=65536;
			tmp->rows=8;
		}
		if(tmp->rows<1)tmp->rows=1;
	}
	if(!tmp->cols)
	{
		for(i=0; i<tmp->num_items; i++)
		{
			j=strlen(tmp->item_value[i]);
			if(j>tmp->cols)tmp->cols=j;
		}
		if(tmp->cols>16)tmp->cols=16;
		if(tmp->cols<4)tmp->cols=4;
	}

	tmp->xs=tmp->cols*8+((tmp->state&65536)?12:0);
	tmp->ys=tmp->rows*8;

	return(0);
}

char *BTGECLUI_WidgetList_GetPropertyS(BTGECLUI_Widget *obj, char *var)
{
	if(!strcmp(var, "value"))return(obj->item_name[obj->curval]);
	return(NULL);
}

int BTGECLUI_WidgetList_SetPropertyS(BTGECLUI_Widget *obj, char *var, char *val)
{
	int i;
	if(!strcmp(var, "value"))
	{
		for(i=0; i<obj->num_items; i++)
			if(!strcmp(obj->item_name[i], val))
		{
			obj->curval=i;
			return(0);
		}
		obj->curval=0;
	}
	return(0);
}

int BTGECLUI_WidgetList_GetPropertyI(BTGECLUI_Widget *obj, char *var)
{
	if(!strcmp(var, "value"))return(obj->curval);
	if(!strcmp(var, "rows"))return(obj->rows);
	if(!strcmp(var, "cols"))return(obj->cols);

	return(0);
}

int BTGECLUI_WidgetList_SetPropertyI(BTGECLUI_Widget *obj, char *var, int val)
{
	if(!strcmp(var, "value"))obj->curval=val;
	if(!strcmp(var, "rows"))obj->rows=val;
	if(!strcmp(var, "cols"))obj->cols=val;
	return(0);
}

int BTGECLUI_WidgetList_AddItem(BTGECLUI_Widget *obj, char *name, char *value)
{
	obj->item_name[obj->num_items]=dystrdup(name);
	obj->item_value[obj->num_items]=dystrdup(value);
	obj->num_items++;

	if(obj->num_items>obj->rows)
		obj->state|=65536;

	return(0);
}

int BTGECLUI_WidgetList_SetItem(BTGECLUI_Widget *obj, char *name, char *value)
{
	int i;

	for(i=0; i<obj->num_items; i++)
		if(!strcmp(obj->item_name[i], name))
	{
		obj->item_value[i]=dystrdup(value);
		return(0);
	}

	obj->item_name[obj->num_items]=dystrdup(name);
	obj->item_value[obj->num_items]=dystrdup(value);
	obj->num_items++;

	if(obj->num_items>obj->rows)
		obj->state|=65536;

	return(0);
}

int BTGECLUI_WidgetList_ClearItems(BTGECLUI_Widget *obj)
{
	obj->num_items=0;
	obj->curval=0;
	obj->state&=~65536;
}

BTGECLUI_WidgetType btgeclui_context->list_methods=
{
NULL,
"list",

BTGECLUI_WidgetList_Render,
BTGECLUI_WidgetList_HandleInput,
NULL,
BTGECLUI_WidgetList_Begin,
BTGECLUI_WidgetList_End,
NULL,
BTGECLUI_WidgetList_GetPropertyS,
BTGECLUI_WidgetList_SetPropertyS,
BTGECLUI_WidgetList_GetPropertyI,
BTGECLUI_WidgetList_SetPropertyI,
NULL,
NULL,
BTGECLUI_WidgetList_AddItem,
BTGECLUI_WidgetList_SetItem,
BTGECLUI_WidgetList_ClearItems
};

int BTGECLUI_WidgetList_Init()
{
	BTGECLUI_Widget_RegisterType(&btgeclui_context->list_methods);
}
