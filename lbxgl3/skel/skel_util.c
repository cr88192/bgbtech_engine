...
LBXGL_Skel2MeshFace *Skel_CloneMeshFace(LBXGL_Skel2MeshFace *face)
{
	...
	tmp=gctalloc("lbxgl_skel_meshface_t", sizeof(LBXGL_Skel2MeshFace));
...
LBXGL_Skel2Mesh *Skel_CloneMesh(LBXGL_Skel2Mesh *mesh)
{
	...
	tmp->face=gctalloc("lbxgl_skel_faces",
		mesh->n_faces*sizeof(LBXGL_Skel2MeshFace *));
	...
	tmp->vecs=gctalloc("lbxgl_skel_vecs",
		mesh->n_vecs*3*sizeof(float));
	for(i=0; i<(mesh->n_vecs*3); i++)
		tmp->vecs[i]=mesh->vecs[i];

	tmp->n_stvecs=mesh->n_stvecs;
	tmp->stvecs=gctalloc("lbxgl_skel_stvecs",
		mesh->n_stvecs*2*sizeof(float));
...
//	if(!prim->vfidx)
	prim->vfidx=gctalloc(
		"lbxgl_skel_vfidx0_d", (prim->n_vecs+16)*sizeof(int));
//	if(!prim->vfnum)
	prim->vfnum=gctalloc(
		"lbxgl_skel_vfnum0_d", (prim->n_vecs+16)*sizeof(int));
...
//	tmp=gctalloc("lbxgl_skel", sizeof(LBXGL_Skel2MeshFace));
//	memset(tmp, 0, sizeof(LBXGL_Skel2MeshFace));
...
void SkelLod_MeshReIndex(LBXGL_Skel2Mesh *mesh)
{
	...
	if(mesh->n_vecs>onxyz)
		oldxyz=gctalloc("lbxgl_skel_newxyz", mesh->n_vecs*3*sizeof(float));
	if(mesh->n_stvecs>onst)
		oldst=gctalloc("lbxgl_skel_newst", mesh->n_stvecs*2*sizeof(float));
