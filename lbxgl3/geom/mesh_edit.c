...
LBXGL_API LBXGL_MeshGroup *lbxgl_mesh_undo[16];
LBXGL_API LBXGL_MeshGroup *lbxgl_mesh_redo[16];


LBXGL_API LBXGL_MeshGroup *LBXGL_Mesh_GetModel()
	{ return(lbxgl_mesh_mdl); }

LBXGL_API void LBXGL_Mesh_SetDirty()
{
	if(lbxgl_mesh_dirty &&
			((lbxgl_state->time_f*1000)>(lbxgl_mesh_lutime+1000)))
		lbxgl_mesh_dirty=0;

	if(lbxgl_mesh_dirty)return;
	LBXGL_Mesh_AddUndo();

	lbxgl_mesh_dirty=1;
	lbxgl_mesh_lutime=lbxgl_state->time_f*1000;
}
...
//	printf("Add Undo\n");
	...
	for(i=0; i<16; i++)
	{
		if(lbxgl_mesh_redo[i])LBXGL_Mesh_DestroyGroup(lbxgl_mesh_redo[i]);
		lbxgl_mesh_redo[i]=NULL;
	}

	if(lbxgl_mesh_undo[15])LBXGL_Mesh_DestroyGroup(lbxgl_mesh_undo[15]);
	for(i=15; i>0; i--)
		lbxgl_mesh_undo[i]=lbxgl_mesh_undo[i-1];
	lbxgl_mesh_undo[0]=LBXGL_Mesh_CloneGroup(lbxgl_mesh_mdl);
	lbxgl_mesh_dirty=0;
}
...
//	printf("Undo\n");
	...
	if(lbxgl_mesh_redo[15])LBXGL_Mesh_DestroyGroup(lbxgl_mesh_redo[15]);
	for(i=15; i>0; i--)
		lbxgl_mesh_redo[i]=lbxgl_mesh_redo[i-1];

	lbxgl_mesh_redo[0]=lbxgl_mesh_mdl;
	lbxgl_mesh_mdl=lbxgl_mesh_undo[0];

	for(i=0; i<15; i++)
		lbxgl_mesh_undo[i]=lbxgl_mesh_undo[i+1];
	lbxgl_mesh_undo[15]=NULL;
...
//	printf("Redo\n");
	...
	lbxgl_mesh_undo[0]=lbxgl_mesh_mdl;
	lbxgl_mesh_mdl=lbxgl_mesh_redo[0];

	for(i=0; i<15; i++)
		lbxgl_mesh_redo[i]=lbxgl_mesh_redo[i+1];
	lbxgl_mesh_redo[15]=NULL;
...
LBXGL_API void LBXGL_Mesh_DrawPrimEditOrtho(LBXGL_MeshPrim *prim)
{
	...
	if(lbxgl_mesh_mode==LBXGL_EDMODE_VERTEX)
	{
		...
		for(i=0; i<prim->n_xyz; i++)
		{
			...
			f=V3F_DIST(pt, io);
			if(f<VEC_SZ)pdglColor4f(1, 0, 1, 1);

			if(prim->fl_xyz[i]&1)pdglColor4f(0, 1, 0, 1);

			pdglBegin(PDGL_LINES);
			lbxgl_mesh_draw_star(prim->xyz+i*3);
			pdglEnd();
		}
	}
	...
	if(lbxgl_mesh_mode==LBXGL_EDMODE_OBJECT)
	{
		...
		if(i)pdglColor4f(1, 0, 1, 1);

		if(prim->fl&1)pdglColor4f(0, 1, 0, 1);
	}
	...
	for(i=0; i<prim->n_face; i++)
	{
		...
		if(lbxgl_mesh_mode==LBXGL_EDMODE_FACE)
		{
			...
			if(j)pdglColor4f(1, 0, 1, 1);

			if(prim->fl_face[i]&1)pdglColor4f(0, 1, 0, 1);
		}
		...
		if(lbxgl_mesh_mode==LBXGL_EDMODE_FACE)pdglColor4f(1, 1, 1, 1);
	}
}
...
LBXGL_API void LBXGL_Mesh_DrawPrimEdit(LBXGL_MeshPrim *prim)
{
	...
	if(lbxgl_cam->ortho)
	{
		LBXGL_Mesh_DrawPrimEditOrtho(prim);
		return;
	}
...
//	fl|=LBXGL_TXFL_ALPHA;
	...
	if((lbxgl_mesh_mode==LBXGL_EDMODE_OBJECT) || (lbxgl_mesh_mode==LBXGL_EDMODE_TEXTURE))
	{
		i=LBXGL_Mesh_CheckLinePrim(prim, lbxgl_cam->morg, lbxgl_cam->mend);
		if(i)
		{
...
//		pdglEnd();
	...
	if((lbxgl_mesh_mode==LBXGL_EDMODE_TEXTURE) && (prim==lbxgl_meshsel_prim) && (prim->texnum>=0))
	{
		pdglEnableTexture2D();
		pdglDisableLighting();
		pdglBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
LBXGL_API void LBXGL_Mesh_DrawGroupEdit(LBXGL_MeshGroup *grp)
{
	...
	while(mcur)
	{
		LBXGL_Mesh_DrawPrimEditAlpha(mcur);
		mcur=mcur->next;
	}
}
...
LBXGL_API void LBXGL_Mesh_ClearOpSelGroup(LBXGL_MeshGroup *grp)
{
	...
	while(gcur)
	{
		LBXGL_Mesh_ClearOpSelGroup(gcur);
		gcur=gcur->next;
	}
	...
	while(mcur)
	{
		LBXGL_Mesh_ClearOpSelPrim(mcur);
		mcur=mcur->next;
	}
}
...
LBXGL_API void LBXGL_Mesh_ClearSel()
{
	...
	LBXGL_Mesh_ClearSelGroup(lbxgl_mesh_mdl);
...
/*	static int vtxb[6*4]=
	...
	while(1)
	{
		...
		prim=LBXGL_Mesh_LookupPrimGroup(lbxgl_mesh_mdl, tb);
		if(!prim)break;
	}

	prim=LBXGL_Mesh_CreatePrim(lbxgl_mesh_mdl, tb);
//	prim=LBXGL_Mesh_CreatePrim(lbxgl_mesh_mdl, "cube");
	V3F_ADDSCALE(lbxgl_cam->org, lbxgl_cam->fw, 10, org);
...
LBXGL_API void LBXGL_Mesh_CmdSelPrim(LBXGL_MeshPrim *prim, int cmd)
{
	...
	case LBXGL_EDCMD_TRANS:
		...
		LBXGL_Mesh_CalcTranslate(io, io);
		for(i=0; i<prim->n_xyz; i++)
	...
	if(lbxgl_mesh_mode==LBXGL_EDMODE_FACE)
	{
		...
		case LBXGL_EDCMD_EXTRUDE:
			LBXGL_Mesh_AddUndo();
			LBXGL_Mesh_ExtrudeFace(prim, i);
			break;
		case LBXGL_EDCMD_SUBDIVIDE:
			LBXGL_Mesh_AddUndo();
			LBXGL_Mesh_DivideFace(prim, i);
			break;
		case LBXGL_EDCMD_INVERT:
			LBXGL_Mesh_AddUndo();
			LBXGL_Mesh_FlipFace(prim, i);
			break;
	...
	if(lbxgl_mesh_mode==LBXGL_EDMODE_VERTEX)
	{
		...
		{
		case LBXGL_EDCMD_TRANS:
			LBXGL_Mesh_CalcTranslate(prim->xyz+i*3, io);
			V3F_ADD(prim->xyz+i*3, io, prim->xyz+i*3);
			break;
	...
	if(lbxgl_mesh_mode==LBXGL_EDMODE_TEXTURE)
	{
		switch(cmd)
		{
		case LBXGL_EDCMD_TRANS:
			LBXGL_Mesh_CalcTranslate(prim->tex_org, io);
			V3F_ADD(prim->tex_org, io, prim->tex_org);
			break;
		case LBXGL_EDCMD_ROT:
			V3F_COPY(prim->tex_org, io);
...
LBXGL_API void LBXGL_Mesh_CmdSelGroup(LBXGL_MeshGroup *grp, int cmd)
{
	...
	while(gcur)
	{
		LBXGL_Mesh_CmdSelGroup(gcur, cmd);
		gcur=gcur->next;
	}
	...
	while(mcur)
	{
		LBXGL_Mesh_CmdSelPrim(mcur, cmd);
		mcur=mcur->next;
	}
}


LBXGL_API void LBXGL_Mesh_BoxSelPrim(LBXGL_MeshPrim *prim,
...
LBXGL_API void LBXGL_Mesh_BoxSelGroup(LBXGL_MeshGroup *grp, float *mins, float *maxs)
{
	...
	while(gcur)
	{
		LBXGL_Mesh_BoxSelGroup(gcur, mins, maxs);
		gcur=gcur->next;
	}
	...
	while(mcur)
	{
		LBXGL_Mesh_BoxSelPrim(mcur, mins, maxs);
		mcur=mcur->next;
	}
}
...
LBXGL_API void LBXGL_Mesh_CmdOpSelPrim(LBXGL_MeshPrim *prim, int cmd, float *org)
{
	...
	{
	case LBXGL_EDCMD_TRANS:
		LBXGL_Mesh_CalcTranslate(org, io);
		for(i=0; i<prim->n_xyz; i++)if(prim->fl_xyz[i]&2)
			{ V3F_ADD(prim->xyz+i*3, io, prim->xyz+i*3); }
		break;

	case LBXGL_EDCMD_ROT:
		f=LBXGL_Mesh_CalcRotate(org);
		for(i=0; i<prim->n_xyz; i++)if(prim->fl_xyz[i]&2)
			LBXGL_Mesh_RotatePointOrgAxisAngle(
				org, lbxgl_cam->fw, -f, prim->xyz+i*3);
		break;
	case LBXGL_EDCMD_SCALE:
		f=LBXGL_Mesh_CalcScale(org);
...
LBXGL_API void LBXGL_Mesh_CmdOpSelGroup(LBXGL_MeshGroup *grp, int cmd, float *org)
{
	...
	while(gcur)
	{
		LBXGL_Mesh_CmdOpSelGroup(gcur, cmd, org);
		gcur=gcur->next;
	}
	...
	while(mcur)
	{
		LBXGL_Mesh_CmdOpSelPrim(mcur, cmd, org);
		mcur=mcur->next;
	}
}
...
LBXGL_API void LBXGL_Mesh_CmdMSelGroup(LBXGL_MeshGroup *grp, int cmd, float *org)
{
	...
	while(gcur)
	{
		LBXGL_Mesh_CmdMSelGroup(gcur, cmd, org);
		gcur=gcur->next;
	}
	...
	while(mcur)
	{
		LBXGL_Mesh_CmdMSelPrim(mcur, cmd, org);
		mcur=mcur->next;
	}
}

LBXGL_API void LBXGL_Mesh_CopyMSelPrim(
...
LBXGL_API void LBXGL_Mesh_CopyMSelGroup(
	LBXGL_MeshGroup *gtmp, LBXGL_MeshGroup *grp, int cmd, float *org)
{
	...
	while(gcur)
	{
		LBXGL_Mesh_CopyMSelGroup(gtmp, gcur, cmd, org);
		gcur=gcur->next;
	}
	...
	while(mcur)
	{
		LBXGL_Mesh_CopyMSelPrim(gtmp, mcur, cmd, org);
		mcur=mcur->next;
	}
}
...
LBXGL_API void LBXGL_Mesh_InsertGroup(LBXGL_MeshGroup *gtmp, LBXGL_MeshGroup *grp, float *org)
{
	...
	while(gcur)
	{
		LBXGL_Mesh_InsertGroup(gtmp, gcur, org);
		gcur=gcur->next;
	}
	...
	while(mcur)
	{
		LBXGL_Mesh_InsertPrim(gtmp, mcur, org);
		mcur=mcur->next;
	}
}
...
LBXGL_API void LBXGL_Mesh_MergeSelGroup(LBXGL_MeshPrim *prim, LBXGL_MeshGroup *grp)
{
	...
	while(gcur)
	{
		LBXGL_Mesh_MergeSelGroup(prim, gcur);
		gcur=gcur->next;
	}
...
LBXGL_API void LBXGL_Mesh_CmdMultiSel(int cmd)
{
	...
	{
		LBXGL_Mesh_CmdMultiSel(LBXGL_EDCMD_COPY);
		LBXGL_Mesh_CmdMultiSel(LBXGL_EDCMD_DELETE);
		return;
	}

	if((lbxgl_mesh_mode==LBXGL_EDMODE_OBJECT) && (cmd==LBXGL_EDCMD_MERGE))
	{
		LBXGL_Mesh_AddUndo();

		prim=LBXGL_Mesh_CreatePrim(lbxgl_mesh_mdl, "merged");
		LBXGL_Mesh_MergeSelGroup(prim, lbxgl_mesh_mdl);
		lbxgl_meshsel_prim=NULL;
		LBXGL_Mesh_ClearSel();
		return;
	}
	...
	LBXGL_Mesh_BoxSelGroup(lbxgl_mesh_mdl, mins, maxs);
	...
	{
		LBXGL_Mesh_CmdMSelGroup(lbxgl_mesh_mdl, cmd, org);
		return;
	}

	if(cmd==LBXGL_EDCMD_COPY)
	{
		if(lbxgl_mesh_clip)LBXGL_Mesh_DestroyGroup(lbxgl_mesh_clip);
		lbxgl_mesh_clip=LBXGL_Mesh_CreateGroup(NULL, "clip");
		LBXGL_Mesh_CopyMSelGroup(lbxgl_mesh_clip, lbxgl_mesh_mdl, cmd, org);
		return;
	}
	...
	if(cmd==LBXGL_EDCMD_DUPLICATE)
	{
		...
		LBXGL_Mesh_CopyMSelGroup(grp, lbxgl_mesh_mdl, cmd, org);

		V3F_COPY(org, io);
		io[0]-=1; io[1]-=1; io[2]-=1;

		LBXGL_Mesh_InsertGroup(lbxgl_mesh_mdl, grp, io);

		return;
	}

	LBXGL_Mesh_ClearOpSelGroup(lbxgl_mesh_mdl);
	LBXGL_Mesh_OpSelGroup(lbxgl_mesh_mdl);
	LBXGL_Mesh_CmdOpSelGroup(lbxgl_mesh_mdl, cmd, org);
}
...
LBXGL_API void LBXGL_Mesh_CmdSel(int cmd)
{
	...
	{
		LBXGL_Mesh_CmdMultiSel(cmd);
		return;
	}

	if(lbxgl_meshsel_multi)
		LBXGL_Mesh_CmdMultiSel(cmd);
	else LBXGL_Mesh_CmdSelGroup(lbxgl_mesh_mdl, cmd);
}
...
//	V3F_ADD(io, lbxgl_cam->rt, io);
	...
	LBXGL_Mesh_BoxGroup(lbxgl_mesh_clip, mins, maxs);
...
//	printf("Paste At (%g %g %g)\n", io[0], io[1], io[2]);
	...
	LBXGL_Mesh_InsertGroup(lbxgl_mesh_mdl, lbxgl_mesh_clip, io);
}
