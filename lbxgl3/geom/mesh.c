...
LBXGL_API LBXGL_MeshGroup *LBXGL_Mesh_LoadModel(char *name)
{
	...
	if(!strcmp(s, "ac"))
	{
		tmp=LBXGL_AC3D_LoadModel(name);
		return(tmp);
	}
//	if(!strcmp(s, "acx"))
//	{
//		tmp=LBXGL_ACXML_LoadModel(name);
//		return(tmp);
//	}
...
LBXGL_API int LBXGL_Mesh_SaveModel(char *name, LBXGL_MeshGroup *grp)
{
	...
	if(!strcmp(s, "ac"))
	{
		i=LBXGL_AC3D_SaveModel(grp, name);
		return(i);
	}
//	if(!strcmp(s, "acx"))
//	{
//		i=LBXGL_ACXML_SaveModel(grp, name);
//		return(i);
//	}
	return(-1);
}
...
{
	LBXGL_MeshGroup *gcur, *gnxt;
	LBXGL_MeshPrim *mcur, *mnxt;

	gcur=grp->first;
	while(gcur)
	{
		gnxt=gcur->next;
		LBXGL_Mesh_DestroyGroup(gcur);
		gcur=gnxt;
	}
	...
	{
		mnxt=mcur->next;
		LBXGL_Mesh_DestroyPrim(mcur);
		mcur=mnxt;
	}
}

LBXGL_API LBXGL_MeshPrim *LBXGL_Mesh_LookupPrimGroup(
	LBXGL_MeshGroup *grp, char *name)
{
	LBXGL_MeshGroup *gcur, *gnxt;
	LBXGL_MeshPrim *mcur, *mnxt, *mtmp;

	gcur=grp->first;
	while(gcur)
	{
		gnxt=gcur->next;
		mtmp=LBXGL_Mesh_LookupPrimGroup(gcur, name);
		if(mtmp)return(mtmp);
		gcur=gnxt;
	}
...
{
	LBXGL_MeshGroup *gcur, *gtmp, *gfst, *glst;
	LBXGL_MeshPrim *mcur, *mtmp, *mfst, *mlst;

	gcur=grp->first; gfst=NULL; glst=NULL;
	while(gcur)
	{
		gtmp=LBXGL_Mesh_CloneGroup(gcur);
	...
	while(mcur)
	{
		mtmp=LBXGL_Mesh_ClonePrim(mcur);
		if(mlst) { mlst->next=mtmp; mlst=mtmp; }
			else { mfst=mtmp; mlst=mtmp; }
		mcur=mcur->next;
	}

	gtmp=gctalloc("lbxgl_meshgroup_t", sizeof(LBXGL_MeshGroup));
...
//	printf("prim %s %d face\n", prim->name, prim->n_face);
...
//			pdglVertex3fv(prim->xyz+prim->pts[(p+j)*2+0]*3);
			...
			LBXGL_Shader_TexCoord2fv(
				prim->st+prim->pts[(p+j)*2+1]*2);
			LBXGL_Shader_Vertex3fv(
				prim->xyz+prim->pts[(p+j)*2+0]*3);
		}
...
//		pdglEnd();
		...
		case 2:
			break;
		case 3:
			LBXGL_Shader_TexCoord2fv(prim->st+prim->pts[(p+0)*2+1]*2);
			LBXGL_Shader_Vertex3fv(prim->xyz+prim->pts[(p+0)*2+0]*3);
			LBXGL_Shader_TexCoord2fv(prim->st+prim->pts[(p+1)*2+1]*2);
			LBXGL_Shader_Vertex3fv(prim->xyz+prim->pts[(p+1)*2+0]*3);
			LBXGL_Shader_TexCoord2fv(prim->st+prim->pts[(p+2)*2+1]*2);
			LBXGL_Shader_Vertex3fv(prim->xyz+prim->pts[(p+2)*2+0]*3);
			break;
		case 4:
			//0
			LBXGL_Shader_TexCoord2fv(prim->st+prim->pts[(p+0)*2+1]*2);
			...
			LBXGL_Shader_TexCoord2fv(prim->st+prim->pts[(p+2)*2+1]*2);
			LBXGL_Shader_Vertex3fv(prim->xyz+prim->pts[(p+2)*2+0]*3);
			LBXGL_Shader_TexCoord2fv(prim->st+prim->pts[(p+3)*2+1]*2);
			LBXGL_Shader_Vertex3fv(prim->xyz+prim->pts[(p+3)*2+0]*3);
			break;
		default:
			for(j=0; j<(n-2); j++)
			{
				LBXGL_Shader_TexCoord2fv(prim->st+prim->pts[(p+0)*2+1]*2);
				LBXGL_Shader_Vertex3fv(prim->xyz+prim->pts[(p+0)*2+0]*3);
				LBXGL_Shader_TexCoord2fv(prim->st+prim->pts[(p+j+1)*2+1]*2);
				LBXGL_Shader_Vertex3fv(prim->xyz+prim->pts[(p+j+1)*2+0]*3);
				LBXGL_Shader_TexCoord2fv(prim->st+prim->pts[(p+j+2)*2+1]*2);
				LBXGL_Shader_Vertex3fv(prim->xyz+prim->pts[(p+j+2)*2+0]*3);
			}
			break;
		}
#endif
...
LBXGL_API void LBXGL_Mesh_DrawPrimDark(LBXGL_MeshPrim *prim)
{
	...
	if(prim->texnum>=0)
	{
		i=LBXGL_Texture_GetImageFlags(prim->texnum);
		j=LBXGL_TXFL_ALPHA|LBXGL_TXFL_VALPHA|LBXGL_TXFL_DETAIL|
			LBXGL_TXFL_WINDOW|LBXGL_TXFL_NODRAW|LBXGL_TXFL_FLUID;
		if(i&j)return;
	}
...
LBXGL_API void LBXGL_Mesh_DrawGroupLight(LBXGL_MeshGroup *grp)
{
	...
	while(gcur)
	{
		LBXGL_Mesh_DrawGroupLight(gcur);
		gcur=gcur->next;
	}
	...
	while(mcur)
	{
		LBXGL_Mesh_DrawPrimLight(mcur);
		mcur=mcur->next;
	}
}

#if 0
LBXGL_API void LBXGL_Mesh_DrawPolyShadowVolume(
	float *xyz, int *pts, int nvec, float *org)
{
	float tva[64*3];
	float tvb[64*3];
	...
	pdglDisableTexture2D();
	pdglColor4f(0, 1, 1, 0.1);

	i=pts[0*2+0]; j=pts[1*2+0]; k=pts[2*2+0];
	V3F_SUB(xyz+i*3, xyz+j*3, dv0);
	V3F_SUB(xyz+k*3, xyz+j*3, dv1);
	V3F_CROSS(dv1, dv0, dvn);
	V3F_NORMALIZE(dvn, dvn);
	dvn[3]=V3F_DOT(xyz+0*3, dvn);
	...
	for(i=0; i<nvec; i++)
	{
		...
		V3F_ADDSCALE(tv0, dv0, 10000, tvb+i*3);
	}
...
// LBXGL_API void LBXGL_Mesh_DrawPrimShadowVolume(LBXGL_MeshPrim *prim, float *org)
{
	float v0[3], v1[3], v2[3];
	float dv0[3], dv1[3], dv2[3];
	int n, p;
	...
	for(i=0; i<prim->n_face; i++)
	{
		...
		LBXGL_Mesh_DrawPolyShadowVolume(
			prim->xyz, prim->pts+p*2, n, org);
...
LBXGL_API void LBXGL_Mesh_DrawPrimShadowVolumeEdge(LBXGL_MeshPrim *prim, float *org)
{
	...
	{
		nxyz=256;
		while(nxyz<=prim->n_xyz)nxyz+=nxyz>>1;
	...
	for(i=0; i<prim->n_edge; i++)
	{
		...
		nv0=prim->norm_face+i0*4;
		nv1=prim->norm_face+i1*4;
	...
	for(i=0; i<prim->n_face; i++)
	{
		...
		nv=prim->norm_face+i*4;
		f=V3F_NDOT(org, nv);
		if(f>0)xyz=xyza; else xyz=xyzb;

		pdglBegin(PDGL_POLYGON);
		for(j=0; j<n; j++)
			pdglVertex3fv(xyz+prim->pts[(p+j)*2+0]*3);
		pdglEnd();
	}
...
LBXGL_API void LBXGL_Mesh_DrawPrimShadowVolumeClip(
	LBXGL_MeshPrim *prim, float *lorg, float lrad)
{
	float v0[3], v1[3], v2[3];
	float dv0[3], dv1[3], dv2[3];
	int pts[16];
	...
	for(i=0; i<prim->n_face; i++)
	{
		...
		for(j=0; j<n; j++)
			pts[j]=prim->pts[(p+j)*2];
		LBXGL_Brush_DrawPolyShadowVolumeDPOrgClip(
			prim->xyz, pts, n, NULL, lorg, lrad, 0);
	}
}

LBXGL_API void LBXGL_Mesh_DrawPrimShadowVolume(
	LBXGL_MeshPrim *prim, float *org, float rad, int flags)
{
	static float xyza[16384*3];
	static float xyzb[16384*3];
	float v0[3], v1[3], v2[3];
	float dv0[3], dv1[3], dvn[4];
	float *tva[16], *tvb[16];
	int *pts;
	...
	if(!prim->edge)LBXGL_Mesh_BuildCache(prim);

	if((flags&1) && LBXGL_Brush_CheckViewSphereShadow(
		prim->com_org, prim->com_rad, org, rad))
	{
		LBXGL_Mesh_DrawPrimShadowVolumeClip(prim, org, rad);
		return;
	}

	if(prim->edge)
	{
		LBXGL_Mesh_DrawPrimShadowVolumeEdge(prim, org);
		return;
	}

	for(i=0; i<prim->n_xyz; i++)
	{
		V3F_COPY(prim->xyz+i*3, v0);
		V3F_SUB(v0, org, dv0);
		V3F_NORMALIZE(dv0, dv0);
		V3F_ADDSCALE(v0, dv0, 1, xyza+i*3);
		V3F_ADDSCALE(v0, dv0, 10000, xyzb+i*3);
	}
	...
	for(i=0; i<prim->n_face; i++)
	{
		...
		i0=pts[0*2+0]; i1=pts[1*2+0]; i2=pts[2*2+0];
		V3F_COPY(xyza+i0*3, v0);
		V3F_COPY(xyza+i1*3, v1);
		V3F_COPY(xyza+i2*3, v2);
		...
		if(f>0)
		{
			for(j=0; j<n; j++)
			{
				tva[j]=xyza+pts[j*2+0]*3;
				tvb[j]=xyzb+pts[j*2+0]*3;
			}
			tva[n]=tva[0];
			tvb[n]=tvb[0];
		}else
		{
			for(j=0; j<n; j++)
			{
				tva[j]=xyza+pts[(n-j-1)*2+0]*3;
				tvb[j]=xyzb+pts[(n-j-1)*2+0]*3;
			}
			tva[n]=tva[0];
			tvb[n]=tvb[0];
		}
		...
		if(n==3)
		{
			...
			pdglVertex3fv(tva[0]); pdglVertex3fv(tva[1]);
			pdglVertex3fv(tva[2]); pdglVertex3fv(tvb[2]);
			pdglVertex3fv(tvb[1]); pdglVertex3fv(tvb[0]);
			pdglEnd();

			pdglBegin(PDGL_QUAD_STRIP);
			pdglVertex3fv(tva[0]); pdglVertex3fv(tvb[0]);
			pdglVertex3fv(tva[1]); pdglVertex3fv(tvb[1]);
			pdglVertex3fv(tva[2]); pdglVertex3fv(tvb[2]);
			pdglVertex3fv(tva[3]); pdglVertex3fv(tvb[3]);
			pdglEnd();
		...
		if(n==4)
		{
			pdglBegin(PDGL_QUADS);
			pdglVertex3fv(tva[0]); pdglVertex3fv(tva[1]);
			pdglVertex3fv(tva[2]); pdglVertex3fv(tva[3]);
			pdglVertex3fv(tvb[3]); pdglVertex3fv(tvb[2]);
			pdglVertex3fv(tvb[1]); pdglVertex3fv(tvb[0]);
			pdglEnd();
			...
			pdglVertex3fv(tva[2]); pdglVertex3fv(tvb[2]);
			pdglVertex3fv(tva[3]); pdglVertex3fv(tvb[3]);
			pdglVertex3fv(tva[4]); pdglVertex3fv(tvb[4]);
			pdglEnd();
		...
		for(j=0; j<n; j++)
		{
			pdglVertex3fv(tva[j+1]);
			pdglVertex3fv(tva[j]);
			pdglVertex3fv(tvb[j]);
			pdglVertex3fv(tvb[j+1]);
		}
		pdglEnd();
#endif
...
LBXGL_API void LBXGL_Mesh_DrawGroupShadowVolume(
	LBXGL_MeshGroup *grp, float *org, float rad, int flags)
{
	LBXGL_MeshGroup *gcur;
	LBXGL_MeshPrim *mcur;
	...
	while(gcur)
	{
		LBXGL_Mesh_DrawGroupShadowVolume(gcur, org, rad, flags);
		gcur=gcur->next;
	}
	...
	while(mcur)
	{
		LBXGL_Mesh_DrawPrimShadowVolume(mcur, org, rad, flags);
		mcur=mcur->next;
	}
}
...
LBXGL_API void LBXGL_Mesh_DrawGroupFinal(LBXGL_MeshGroup *grp)
{
	...
	while(gcur)
	{
		LBXGL_Mesh_DrawGroupFinal(gcur);
		gcur=gcur->next;
	}
...
LBXGL_API void LBXGL_Mesh_DrawPrimAlpha(LBXGL_MeshPrim *prim)
{
	...
	if(prim->texnum>=0)
	{
		i=LBXGL_Texture_GetImageFlags(prim->texnum);
		j=LBXGL_TXFL_ALPHA|LBXGL_TXFL_VALPHA|LBXGL_TXFL_DETAIL|
			LBXGL_TXFL_WINDOW|LBXGL_TXFL_FLUID;
		if(!(i&j))return;
	}
...
LBXGL_API void LBXGL_Mesh_DrawGroupAlpha(LBXGL_MeshGroup *grp)
{
	...
	while(gcur)
	{
		LBXGL_Mesh_DrawGroupAlpha(gcur);
		gcur=gcur->next;
	}
	...
	while(mcur)
	{
		LBXGL_Mesh_DrawPrimAlpha(mcur);
		mcur=mcur->next;
	}
}


LBXGL_API LBXGL_MeshPrim *LBXGL_Mesh_CreatePrim(
	LBXGL_MeshGroup *up, char *name)
{
	LBXGL_MeshPrim *prim;

	prim=gctalloc("lbxgl_meshprim_t", sizeof(LBXGL_MeshPrim));
...
LBXGL_API LBXGL_MeshGroup *LBXGL_Mesh_CreateGroup(
	LBXGL_MeshGroup *up, char *name)
{
	LBXGL_MeshGroup *tmp;
...
LBXGL_API int LBXGL_Mesh_AllocPTS(LBXGL_MeshPrim *prim, int num)
{
	...
	LBXGL_Mesh_CheckExpandPTS(prim, num);
	i=prim->n_pts;
	prim->n_pts+=num;
	for(j=0; j<num; j++)
		prim->fl_pts[i+j]=0;
...
LBXGL_API int LBXGL_Mesh_AddTriangle(LBXGL_MeshPrim *prim,
...
LBXGL_API int LBXGL_Mesh_AddQuad(LBXGL_MeshPrim *prim,
...
LBXGL_API int LBXGL_Mesh_AddPolygon(LBXGL_MeshPrim *prim,
...
LBXGL_API void LBXGL_Mesh_RotatePointOrgAxisAngle(
	float *o, float *n, float th, float *pt)
{
	V3F_SUB(pt, o, pt);
	LBXGL_Mesh_RotatePointAxisAngle(n, th, pt);
	V3F_ADD(pt, o, pt);
}
...
LBXGL_API void LBXGL_Mesh_RotateFace(LBXGL_MeshPrim *prim, int fn,
...
LBXGL_API void LBXGL_Mesh_BoxFace(LBXGL_MeshPrim *prim, int fn,
...
LBXGL_API void LBXGL_Mesh_DeleteVertex(LBXGL_MeshPrim *prim, int vn)
{
	...
	for(i=0; i<prim->n_face; i++)
	{
		j=LBXGL_Mesh_CheckFaceVertex(prim, i, vn);
		if(j)LBXGL_Mesh_DeleteFace(prim, i);
	}
	prim->fl_xyz[vn]|=128;
}

LBXGL_API int LBXGL_Mesh_CheckLineFace(
	LBXGL_MeshPrim *prim, int fn, float *src, float *dst)
{
	float v0[3], v1[3], v2[3];
	float dv0[3], dv1[3], dv2[3];
...
LBXGL_API int LBXGL_Mesh_CheckLinePrim(
	LBXGL_MeshPrim *prim, float *src, float *dst)
{
	int i, j;
	...
	for(i=0; i<prim->n_face; i++)
	{
		...
		j=LBXGL_Mesh_CheckLineFace(prim, i, src, dst);
		if(j)return(1);
	}
	return(0);
}
...
LBXGL_API void LBXGL_Mesh_BoxGroup(LBXGL_MeshGroup *grp, float *mins, float *maxs)
{
	...
	while(gcur)
	{
		LBXGL_Mesh_BoxGroup(gcur, mins, maxs);
		gcur=gcur->next;
	}
	...
	while(mcur)
	{
		LBXGL_Mesh_BoxPrim(mcur, mins, maxs);
		mcur=mcur->next;
	}
}
...
//	printf("%d OP, %d FP, %d FE, %d OE\n", nop, nfp, nfe, noe);
	...
	for(i=0; i<nfp; i++)
		for(j=0; j<nfe; j++)
			if(fe[j*2+0]==i)
	{
		eia[0]=LBXGL_Mesh_IndexXYZ(prim, fp+i*3);
		eib[0]=LBXGL_Mesh_IndexST(prim, fpst+i*2);
		k=fe[j*2+1];
		if(k<0)k=(-k)-1;

		eia[1]=LBXGL_Mesh_IndexXYZ(prim, ep+k*3);
		eib[1]=LBXGL_Mesh_IndexST(prim, epst+k*2);

		i0=oe[k*2+((fe[j*2+1]<0)?0:1)];
		eia[2]=LBXGL_Mesh_IndexXYZ(prim, op+i0*3);
		eib[2]=LBXGL_Mesh_IndexST(prim, opst+i0*2);
		...
		if(l>=noe)
		{
			LBXGL_Mesh_AddTriangle(prim, eia[0], eib[0],
				eia[1], eib[1], eia[2], eib[2]);
			continue;
		}

		eia[3]=LBXGL_Mesh_IndexXYZ(prim, ep+l*3);
		eib[3]=LBXGL_Mesh_IndexST(prim, epst+l*2);
...
//			eia[2], eib[2], eia[3], eib[3]);
		...
		LBXGL_Mesh_AddQuad(prim, eia[0], eib[0], eia[1], eib[1],
			eia[2], eib[2], eia[3], eib[3]);
	}
...
LBXGL_API int LBXGL_Mesh_OptimizePrim(LBXGL_MeshPrim *prim)
{
	...
	for(i=0; i<n_face; i++)
	{
		...
		p1=LBXGL_Mesh_AllocPTS(prim, n);
		for(j=0; j<n; j++)
		{
			fa=xyz+pts[(p+j)*2+0]*3;
			fb=st+pts[(p+j)*2+1]*2;
			prim->pts[(p1+j)*2+0]=LBXGL_Mesh_IndexXYZ(prim, fa);
			prim->pts[(p1+j)*2+1]=LBXGL_Mesh_IndexST(prim, fb);
		}
...
LBXGL_API void LBXGL_Mesh_ProjectPointST(LBXGL_MeshPrim *prim,
...
LBXGL_API int LBXGL_Mesh_LoadTexture(
	LBXGL_MeshPrim *prim, int *rw, int *rh)
{
	char tb[256];
	int i, w, h;
...
//	i=Tex_LoadFile(prim->tex, &w, &h);
	i=LBXGL_Texture_LoadImageSize(prim->tex, &w, &h);
	if(i<0)
	{
		sprintf(tb, "textures/%s", prim->tex);
//		i=Tex_LoadFile(tb, &w, &h);
		i=LBXGL_Texture_LoadImageSize(tb, &w, &h);
	}
	...
	if(!prim->norm_face)
	{
		...
		for(i=0; i<prim->n_face; i++)
		{
			...
			for(j=0; j<n; j++)
			{
				if((p+j)>=prim->n_pts)
				{
					printf("LBXGL_Mesh_BuildCache: overflow trap\n");
					continue;
				}
