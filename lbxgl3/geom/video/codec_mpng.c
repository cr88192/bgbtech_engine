...
int LBXGL_MPNG_DecompressFrame(LBXGL_AVI_Context *ctx,
...
int lbxgl_mpng_decompress_frame(LBXGL_VidCodecCTX *ctx,
	void *src, void *dst, int ssz, int dsz)
{
	vfw_ctxinfo *info;
...
static LBXGL_VidCodecCTX *lbxgl_mpng_begin_decompress(int fcc,
...
//	info->buffer=malloc(out->width*out->height*out->bpp/8);
	...
	ctx->decompress_frame=&lbxgl_mpng_decompress_frame;
...
int LBXGL_CodecMPNG_Init()
{
	...
	codec->begin_decompress=&lbxgl_mpng_begin_decompress;
	return(0);
}
