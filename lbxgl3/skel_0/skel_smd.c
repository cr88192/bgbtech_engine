...
LBXGL_API int LBXGL_SkelSmd_LoadBaseBones(LBXGL_Skel2Model *mdl, VFILE *fd)
{
	...
	if(!mdl->max_bones)
	{
		...
		mdl->bone_name=gcalloc(mdl->max_bones*sizeof(char *));
		mdl->bone_up=gcalloc(mdl->max_bones*sizeof(int));
		mdl->bone_org=gcalloc(mdl->max_bones*3*sizeof(float));
		mdl->bone_rot=gcalloc(mdl->max_bones*9*sizeof(float));
		mdl->bone_ang=gcalloc(mdl->max_bones*3*sizeof(float));
	}
...
LBXGL_API LBXGL_Skel2Mesh *LBXGL_SkelSmd_BuildMesh()
{
	...
	tmp->face=gcalloc(skelsmd_num_face*sizeof(LBXGL_Skel2MeshFace *));
	...
	tmp->vecs=gcalloc(skelsmd_num_xyz*3*sizeof(float));
	tmp->stvecs=gcalloc(skelsmd_num_st*2*sizeof(float));
	...
	tmp->vfidx=gcalloc(tmp->n_vecs*sizeof(int));
	tmp->vfnum=gcalloc(tmp->n_vecs*sizeof(int));
	...
	tmp->vfids=gcalloc(n*sizeof(int));
...
LBXGL_API int LBXGL_SkelSmd_LoadTriangles(LBXGL_Skel2Model *mdl, VFILE *fd)
{
	...
	if(!skelsmd_face_xyz)
	{
		...
		skelsmd_skinname=gcalloc(256*sizeof(char *));
	}
