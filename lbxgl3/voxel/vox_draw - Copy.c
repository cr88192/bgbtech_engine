...
void LBXGL_Voxel_DrawChunkSimpleAlphaFlat(LBXGL_VoxelChunk *chk)
{
	if(!chk->n_aface)
		return;
	LBXGL_Shader_DrawArraysSimpleFlat(GL_QUADS, 0, chk->n_aface*4,
		3, GL_FLOAT, 0, chk->aface_xyz);
}
...
//	if(!chk->n_face && !chk->n_aface)
	...
	LBXGL_Shader_DrawArraysNormalFlat(GL_QUADS, 0, chk->n_aface*4,
		3, GL_FLOAT, 0, chk->aface_xyz,
		3, GL_BYTE, 3, chk->aface_norm);
}
