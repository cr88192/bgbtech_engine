...
//	BSP_ReadFourcc(fd);
	...
	tmp->vecs=kalloc(tmp->nvecs*3*sizeof(float));
	for(i=0; i<tmp->nvecs; i++)
		BSP_ReadVector(fd, tmp->vecs+i*3);

	tmp->evecs=kalloc(4*tmp->nvecs*sizeof(float));
	for(i=0; i<tmp->nvecs; i++)
	{
		j=(i+1)%tmp->nvecs;
...
//	BSP_ReadFourcc(fd);
	...
	tmp->vecs=kalloc(tmp->nvecs*3*sizeof(float));
	tmp->st=kalloc(tmp->nvecs*2*sizeof(float));
	tmp->uv=kalloc(tmp->nvecs*2*sizeof(float));
	if(tmp->lm)tmp->vidx=kalloc(tmp->nvecs*sizeof(int));
	for(i=0; i<tmp->nvecs; i++)
	{
		BSP_ReadVector(fd, tmp->vecs+i*3);
...
//	BSP_ReadFourcc(fd);
	...
	tmp->vec_xyz=kalloc(tmp->num_vecs*3*sizeof(float));
	tmp->vec_uv=kalloc(tmp->num_vecs*2*sizeof(float));
...
//	printf("Read LM %dx%d\n", tmp->xs, tmp->ys);
	...
	if(tmp->fl&BSPLM_FL_MONO)
	{
		...
		for(j=0; j<tmp->dn; j++)
		{
			...
			tmp->dlmbuf[j]=kalloc(tmp->dxs*tmp->dys);
	...
	{
		tmp->lmbuf=malloc(tmp->xs*tmp->ys*4);
		...
		for(j=0; j<tmp->dn; j++)
		{
			...
			tmp->dlmbuf[j]=kalloc(tmp->dxs*tmp->dys*4);
	...
	buf=tmp->lmbuf;
//	buf2=malloc(tmp->xs*tmp->ys);
	if(!buf2)buf2=malloc(2048*2048);
...
BSP_Model *BSP_ReadModel(VFILE *fd)
{
	...
	tmp=ObjType_New("bsp_model_t", sizeof(BSP_Model));
...
//	BSP_ReadInt32(fd, 18);
	...
	while(1)
	{
		...
		if(i==FCC_tExi)
		{
			...
			tmp->texnames=kalloc(tmp->num_tex*sizeof(char *));
			tmp->tex_xs=kalloc(tmp->num_tex*sizeof(int));
			tmp->tex_ys=kalloc(tmp->num_tex*sizeof(int));
			tmp->tex_fl=kalloc(tmp->num_tex*sizeof(int));
			tmp->tex_num=kalloc(tmp->num_tex*sizeof(int));
			tmp->tex_num2=kalloc(tmp->num_tex*sizeof(int));
...
LBXGL_World *BSP_ReadBSP(char *name)
{
	...
	tmp=ObjType_New("lbxgl_world_t", sizeof(LBXGL_World));
	...
	while(1)
	{
		...
		{
			j=BSP_ReadInt32(fd);
			tmp->styles[tmp->num_styles]=kalloc(j+1);
...
//			mcur->ent=cur;
		...
		{
			ltmp=kalloc(sizeof(LBXGL_Light));
