...
{
	BTGE_BrushPatchPoint *pt11, *pt12, *pt13;
	BTGE_BrushPatchPoint *pt21, *pt22, *pt23;
	BTGE_BrushPatchPoint *pt31, *pt32, *pt33;
...
//	x=(int)floor(xpos); y=(int)floor(ypos);
	...
	BTGE_BrushPatch_InterpolateVecPts(
...
//	ysc=(brush->patch->ny_pts)/((float)ys);
		...
		for(j=0; j<(xs-1); j++)
	{
		...
		BTGE_BrushPatch_CalcNormalPts(p0, p1, p2, p0->norm);
		V3F_COPY(p0->norm, p1->norm);
		V3F_COPY(p0->norm, p2->norm);
		V3F_COPY(p0->norm, p3->norm);
		p0->norm[3]=V3F_DOT(p0->xyz, p0->norm);
		p1->norm[3]=V3F_DOT(p1->xyz, p1->norm);
		p2->norm[3]=V3F_DOT(p2->xyz, p2->norm);
		p3->norm[3]=V3F_DOT(p3->xyz, p3->norm);
	}
