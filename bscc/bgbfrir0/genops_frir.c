...
int main(int argc, char *argv[])
{
	...
	in_txt="bsvm_ops.txt";
	out_c="base/frir_ops.c";
	out_h="include/bgbfrir_ops.h";
	out_suf="";
	out_suf2="S";
	out_pfx="FRIR";
	...
	for(i=1; i<argc; i++)
	{
		if(argv[i][0]=='-')
		{
			...
			if(!strcmp(argv[i], "-Oh"))
			{
				out_h=strdup(argv[i+1]);
				i++; continue;
			}

			if(!strcmp(argv[i], "-pfx"))
			{
				out_pfx=strdup(argv[i+1]);
				i++; continue;
			}
	...
	while(*s) { *t++=toupper(*s++); }
	*t++=0;
	out_pfx_uc=strdup(buf);

	s=out_pfx; t=buf;
	while(*s) { *t++=tolower(*s++); }
	*t++=0;
	out_pfx_lc=strdup(buf);

#if 0
	if(argc>1)
	{
		sprintf(buf, "bsvm_ops%s.txt", argv[1]);
		in_txt=strdup(buf);

		sprintf(buf, "core/bsvm_ops%s.c", argv[1]);
		out_c=strdup(buf);

		sprintf(buf, "include/bsvm_ops%s.h", argv[1]);
		out_h=strdup(buf);
	...
	fprintf(fd, "int %s_ops%s_nops=%d;\n\n", out_pfx_lc, out_suf, n);

	fprintf(fd, "char *%s_ops%s_strs[]={\n", out_pfx_lc, out_suf);
	j=0;
	for(i=0; i<n; i++)
	{
		s=ops[i];
	...
	for(i=0; i<n; i++)if(ops[i])
	{
		...
		j=(31-strlen(buf))/8;
		fprintf(fd, "#define %s_%sOP_%s", out_pfx_uc, out_suf2, buf, i);
		while(j--)fprintf(fd, "\t");
		fprintf(fd, "%d\n", i);
	}
