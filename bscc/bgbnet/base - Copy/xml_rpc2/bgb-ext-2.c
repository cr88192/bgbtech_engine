...
int XMLRPC_EXT2_PrintDate(char *s, NET_DateTime *dt)
{
	...
	s=kprints(s, "%04d-%02d-%02d", y, dt->month, dy->day);
	
	s=kprints(s, "T%02d:%02d:%02d",
	...
	if(dt->tz_h || dt->tz_m)
	{
		...
		s=kprints(s, "%02d:%02d", tzh, dt->tz_m);
	}else *s++='Z';
...
NetParse_Node *xmlrpc_ext2_encode(XMLRPC_Context *ctx, void *val)
{
	NetParse_Node *t, *t2;
	char buf[16];
	byte *s, *s2;
...
void *xmlrpc_ext2_decode(XMLRPC_Context *ctx, NetParse_Node *exp)
{
	void *p;
	char *s, *t;
	int i, j;
