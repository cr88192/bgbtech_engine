...
XMPP_MucRoom *XMPP_MucRoom_NewRoom(char *room, char *host, char *nick)
{
	...
	tmp=kalloc(sizeof(XMPP_MucRoom));

	tmp->users=kalloc(1024*sizeof(char *));

	tmp->room=kstrdup(room);
	tmp->host=kstrdup(host);
	tmp->nick=kstrdup(nick);
...
XMPP_MucRoom *XMPP_MucRoom_Join(char *room, char *host, char *nick)
{
	...
	kprints(buf, "%s@%s/%s", room, host, nick);

	kprint("join room '%s'\n", buf);

	n1=dyxNewNode();
	n1->key=kstrdup("history");
	dyxAddAttr(n1, "maxchars", "0");

	n=dyxNewNode();
	n->key=kstrdup("x");
	n->first=n1;

	dyxAddAttr(n, "xmlns", "http://jabber.org/protocol/muc");
...
int XMPP_MucRoom_Leave(XMPP_MucRoom *room)
{
	...
	kprints(buf, "%s@%s/%s", room->room, room->host, room->nick);
...
char *XMPP_MucRoom_UserJid(XMPP_MucRoom *room, char *nick)
{
	...
	kprints(buf, "%s@%s/%s", room->room, room->host, nick);
	return(krstrdup(buf));
}
...
int XMPP_MucRoom_SendMessage(XMPP_MucRoom *room, dyxNode *body)
{
	...
	kprints(buf, "%s@%s", room->room, room->host);
