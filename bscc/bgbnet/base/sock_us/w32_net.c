...
VADDR *NET_Str2Addr(char *str, int proto)
{
	...
	case PROTO_IPV4TCP:
		...
		tmp->ipv4.addr=htonl(str2ipv4(str, &i));
		tmp->ipv4.port=i;
...
//	err = WSAStartup(MAKEWORD( 1, 1 ), &wsaData);
	...
	if (err != 0)
	{
		gc_printf("Winsock: winsock.dll error.\n");
		return(-1);
	}
...
//		HIBYTE( wsaData.wVersion ) != 1 )
	...
	{
		gc_printf("Winsock: incompatible winsock version.\n");
		WSACleanup();
		return(-1);
	}
...
Cat pdnet;NET;Sock
Form
	VADDR *NET_DecodeHostnamePort(char *name, int defport);
Description
	Identifies the protocol and decodes the address if possible, otherwise assuming it is a hostname and performing a lookup.
	Defport identifies the default port number (used if no explicit port is given).
--*/
