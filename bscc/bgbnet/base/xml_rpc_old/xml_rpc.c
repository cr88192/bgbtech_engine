...
//		TyFcn_DumpElem(t);
	...
	gc_printf("handle call\n");

	t=XmlRpc_HandleCall(t);

	gc_printf("encode response\n");
	t=XmlRpc_EncodeResponse(t);

	gc_printf("send response\n");

	buf=gcalloc(16384);
	s=buf;

	s=gcrsprints(s, "<?xml version=\"1.0\"?>\n");
