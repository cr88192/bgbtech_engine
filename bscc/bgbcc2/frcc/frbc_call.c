	...
	BGBCC_FrCC_State *ctx, frbc_register reg)
{
	...
	if(BGBCC_FrBC_IsRegRegExtP(ctx, reg))
	{
		...
		j=-1;
		if(BGBCC_FrBC_TypeSmallIntP(ctx, ty))	j=0;
		if(BGBCC_FrBC_TypeSmallLongP(ctx, ty))	j=1;
		if(BGBCC_FrBC_TypeFloatP(ctx, ty))		j=2;
		if(BGBCC_FrBC_TypeDoubleP(ctx, ty))		j=3;
		if(BGBCC_FrBC_TypePointerP(ctx, ty))	j=4;
		if(BGBCC_FrBC_TypeArrayP(ctx, ty))		j=4;
