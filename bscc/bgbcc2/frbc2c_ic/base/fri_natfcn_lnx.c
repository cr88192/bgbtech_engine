...
//		if(j>sz)sz=j;
		...
		FR2CI_FFI_AddExeSection((char *)info->dlpi_name,
			addr, sz, 0);
	}
...
//		(s64)info->dlpi_addr, sz, 0);
	...
	FR2CI_FFI_LoadDynamicOS((char *)info->dlpi_name);
...
char *FR2CI_FFI_LookupAddrNameOS(void *addr, void **addr2)
{
	...
	s=fr2ci_strdup((char *)inf.dli_sname);
	return(s);
...
//	hdl=dlopen(name, RTLD_LAZY);
	...
	printf("FR2CI_FFI_LoadDynamicOS: Fail %s\n", dlerror());
...
//	buf=blnk_loadfile(name, &sz);
	buf=FR2CI_FFI_LoadFileRawOS(name, &sz);
	if(buf)
	{
		printf("FR2CI_FFI_LoadModuleFileDataOS: Got %s Ptr=%p Sz=%d\n",
			name, buf, sz);
		if(rsz)*rsz=sz;
		return(buf);
	}
...
//	*(int *)-1=-1;
	printf("FR2CI_FFI_LoadModuleFileDataOS: Fail %s\n", name);
