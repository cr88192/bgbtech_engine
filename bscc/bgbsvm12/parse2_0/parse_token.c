...
char *bsvmp_pbasestack[16];
char *bsvmp_fnamestack[16];
int bsvmp_lnumstack[16];
int bsvmp_lnumstackpos;

extern char *bsvmp_curfilename;
extern int bsvmp_curlinenum;

BGBSVM_API int BSVMP_GetLinenum()
	{ return(bsvmp_linenum); }
...
int BSVMP_PushLinenum()
{
	...
	bsvmp_lnumstack[bsvmp_lnumstackpos]=
		bsvmp_linenum;
	bsvmp_pbasestack[bsvmp_lnumstackpos]=
		bsvmp_pbase;
	bsvmp_fnamestack[bsvmp_lnumstackpos]=
		bsvmp_fname;
	bsvmp_lnumstackpos++;
	return(0);
}
...
static int BSVM_MSG_Print(char *se, char *ty, char *s, va_list lst)
{
	...
	if(bsvmp_fname)
	{
		sprintf(t, "%s: ", bsvmp_fname);
		t+=strlen(t);
	}
	sprintf(t, "%s: %d: ", ty, BSVMP_CalcLinenum(se));
	t+=strlen(t);
...
{
	if((c>='0') && (c<='9'))return(1);
	if(BSVMP_NameInitChar(c))return(1);
	return(0);
}
...
//			*t++=*s++;
		...
		*ty=BSVM_TOKEN_STRING;
		if(i)*ty=BSVM_TOKEN_CHARSTRING;
		return(s);
	}
...
//		kprint("token: %p '%s'\n", s, b);
		*ty=BSVM_TOKEN_OPERATOR;
		if(!strcmp(b, ":"))*ty=BSVM_TOKEN_SEPERATOR;
		if(!strcmp(b, "::"))*ty=BSVM_TOKEN_SEPERATOR;

		return(s);
	}

	BSVM_MSG_Error(s, "BSVMP_Token: Parse Error Char '%c'\n", *s);
	*ty=BSVM_TOKEN_NULL;
	return(NULL);
}
...
char *BSVMP_TokenQName(char *s, char *b, int *rty)
{
	...
	s1=BSVMP_Token(s, tb2, &ty2);
	s1=BSVMP_Token(s1, tb, &ty);
	while(
		((!strcmp(tb2, "::") && (ty2==BSVM_TOKEN_SEPERATOR)) ||
		(!strcmp(tb2, ".") && (ty2==BSVM_TOKEN_SEPERATOR))) &&
		(ty==BSVM_TOKEN_NAME))
	{
		*t++='/';
