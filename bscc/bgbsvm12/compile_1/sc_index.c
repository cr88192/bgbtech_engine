...
BGBSVM_API int BSVMC_IndexLit(BSVM_SVMCState *ctx, dytf l)
{
	int i;

	if(dytfNullP(l))return(0);
	for(i=1; ctx->lit[i]; i++)
		if(dytfEqP(dytfWrap(ctx->lit[i]), l))
			return(i);
	ctx->lit[i]=dytfUnwrap(l);
	return(i);
}

BGBSVM_API int BSVMC_IndexLit2(BSVM_SVMCState *ctx, dytf l)
{
	int i;

	if(dytfNullP(l))return(0);
	for(i=1; ctx->lit[i]; i++)
		if(dytfEqqP(dytfWrap(ctx->lit[i]), l))
			return(i);
	ctx->lit[i]=dytfUnwrap(l);
	return(i);
}
...
BGBSVM_API void BSVMC_EncIndexLit(BSVM_SVMCState *ctx, dytf l)
{
	int i;
...
BGBSVM_API void BSVMC_EncIndexLit2(BSVM_SVMCState *ctx, dytf l)
{
	int i;
...
int BSVMC_RegIdx(dytf t)
{
	char *s;

	if(!dytfSymbolP(t))return(-1);
	s=dytfSymbolv(t);
	if(s[0]!='$')return(-1);
...
BGBSVM_API void BSVMC_CompileJmpSwitchEqual(
	BSVM_SVMCState *ctx, dytf v, char *s)
{
	if(dytfFixintP(v))
...
BGBSVM_API void BSVMC_CompileJCO(
	BSVM_SVMCState *ctx, int op, dytf ln, dytf rn)
{
	dytf lt, rt;
	dytf t0, t1;
	int i, j, k;
	...
	if(BSVMC_TypeFixnumP(ctx, lt) && (j>=0) && dyFixintP(rn) &&
		(dytfIntv(rn)>-128) && (dytfIntv(rn)<128) &&
		!ctx->i_cap)
	{
		switch(op)
		{
...
//	if(BSVMC_TypeSmallIntP(ctx, lt) && dyintp(rn))
	if(BSVMC_TypeSmallDoubleP(ctx, lt) && dytfIntP(rn))
	{
		switch(op)
		{
...
//	if(BSVMC_TypeSmallIntP(ctx, lt) && BSVMC_TypeSmallIntP(ctx, rt))
	if(BSVMC_TypeSmallDoubleP(ctx, lt) && dytfEqP(lt, rt))
	{
		switch(op)
		{
...
//	if(BSVMC_TypeFixnumP(ctx, lt) && (rn==dyIntF(0)))
	if(BSVMC_TypeSmallFixIntP(ctx, lt) && dytfEqP(rn, dytfInt(0)))
	{
		switch(op)
		{
...
BGBSVM_API void BSVMC_CompileJCT(BSVM_SVMCState *ctx, dytf l)
{
	dytf t;
	int i;
	...
	if(BSVMC_FormIs(l, "unary") && dytfEqP(dytfCadr(l), dytfSymbol("!")))
	{
...
//		if(t==dykeyword("int"))
	...
	BSVMC_CompileJCO(ctx, i, dytfCaddr(l), dytfCadddr(l));
}

BGBSVM_API void BSVMC_CompileJCF(BSVM_SVMCState *ctx, dytf l)
{
	dytf t;
	int i;
	...
	if(BSVMC_FormIs(l, "unary") && dytfEqP(dytfCadr(l), dytfSymbol("!")))
	{
		BSVMC_CompileJCT(ctx, dytfCaddr(l));
		return;
	}
