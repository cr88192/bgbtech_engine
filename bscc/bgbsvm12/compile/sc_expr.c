...
void BSVMC_CompileForm(BSVM_SVMCState *ctx, dyt l)
{
	...
	if(dyFormIsP(l, "label"))
	{
		if(!ctx->tail)
		{
			dyDPrintf("invalid use of label\n");
			return;
		}
...
#if 0
		if(dyconsp(c) && i)dyDPrintf("CCAP: %s\n",
			BSVM_TY_GetSymbol(l));
		if(dyconsp(c) && !i)dyDPrintf("LOC: %s\n",
			BSVM_TY_GetSymbol(l));
		if(!dyconsp(c))dyDPrintf("OBJ: %s\n",
			BSVM_TY_GetSymbol(l));
#endif
