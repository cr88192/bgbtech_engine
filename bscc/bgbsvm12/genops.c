...
int main(int argc, char *argv[])
{
	...
	in_txt="bsvm_ops.txt";
	out_c="svm/bsvm_ops.c";
	out_h="include/bsvm_ops.h";
	out_suf="";
	out_suf2="S";
	...
	fprintf(fd, "int bsvm_ops%s_nops=%d;\n\n", out_suf, n);

	fprintf(fd, "char *bsvm_ops%s_strs[]={\n", out_suf);
	j=0;
	for(i=0; i<n; i++)
	{
		s=ops[i];
	...
	fprintf(fd, "char *bsvm_ops%s_args[]={\n", out_suf);
	j=0;
	for(i=0; i<n; i++)
	{
		s=args[i];
	...
	fprintf(fd, "char *bsvm_ops%s_sfis[]={\n", out_suf);
	j=0;
	for(i=0; i<n; i++)
	{
		s=sfis[i];
	...
	for(i=0; i<n; i++)if(ops[i])
	{
		...
		j=(31-strlen(buf))/8;
		fprintf(fd, "#define BSVM_%sOP_%s", out_suf2, buf, i);
		while(j--)fprintf(fd, "\t");
		fprintf(fd, "%d\n", i);
	}
