...
extern elem bsvm_svm_toplevel;

BSVM_SVMState bsvm_jit_svmstate;	//example VM state
#define JIT_OFFS(n)	(((byte *)(&(bsvm_jit_svmstate.n)))-((byte *)(&bsvm_jit_svmstate)))


elem bsvm_jit_temp[16];		//temp "registers"
BSVM_JitContext *bsvm_jit_ctx;
int bsvm_jit_seq=1;


elem BSVM_JIT_PopDict(BSVM_SVMState *ctx)
{
	elem t, u, v;
	...
	while(BSVM_CONSP(v))
	{
		BSVM_SObj_BindSlot(u, CAR(v), CADR(v));
		v=CDDR(v);
	}
	BSVM_GC_CheckRef(t);
	return(u);
...
elem BSVM_JIT_PopVector(BSVM_SVMState *ctx)
{
	elem t, u, v;
...
elem BSVM_JIT_PopMatrix(BSVM_SVMState *ctx)
{
	elem t, u, v;
...
elem BSVM_JIT_AddFl(elem a, elem b) { return(FLONUM(TOFLOAT(a)+TOFLOAT(b))); }
elem BSVM_JIT_SubFl(elem a, elem b) { return(FLONUM(TOFLOAT(a)+TOFLOAT(b))); }
elem BSVM_JIT_MulFl(elem a, elem b) { return(FLONUM(TOFLOAT(a)+TOFLOAT(b))); }
elem BSVM_JIT_DivFl(elem a, elem b) { return(FLONUM(TOFLOAT(a)+TOFLOAT(b))); }
elem BSVM_JIT_NegFl(elem a) { return(FLONUM(-TOFLOAT(a))); }

int BSVM_JIT_CmpEFl(elem a, elem b) { return(TOFLOAT(a)==TOFLOAT(b)); }
int BSVM_JIT_CmpNeFl(elem a, elem b) { return(TOFLOAT(a)!=TOFLOAT(b)); }
int BSVM_JIT_CmpLFl(elem a, elem b) { return(TOFLOAT(a)<TOFLOAT(b)); }
int BSVM_JIT_CmpGFl(elem a, elem b) { return(TOFLOAT(a)>TOFLOAT(b)); }
int BSVM_JIT_CmpLeFl(elem a, elem b) { return(TOFLOAT(a)<=TOFLOAT(b)); }
int BSVM_JIT_CmpGeFl(elem a, elem b) { return(TOFLOAT(a)>=TOFLOAT(b)); }

elem BSVM_JIT_DivFn(elem a, elem b) { return(FLONUM(TOFLOAT(a)/TOFLOAT(b))); }


int BSVM_JIT_RetCleanup(BSVM_SVMState *ctx)
{
	elem t;

#ifdef BSVM_BLKSTKARGS
	if(ctx->klenv && !ctx->vlenv)
	{
		t=BSVM_SVM_PopF(ctx);
		BSVM_SVM_PopMark(ctx);
//		ctx->args=ctx->markstack[ctx->markstackpos-1];
//		ctx->eargs=ctx->stackpos;
		BSVM_SVM_PushF(ctx, t);
	}
#endif
...
elem _cdecl BSVM_JIT_Lookup(BSVM_SVMState *ctx, elem key)
{
	*(int *)-1=-1;
}
...
void BSVM_JIT_Init()
{
	...
	BASM_RegisterPtr("PopDict", &BSVM_JIT_PopDict);
	BASM_RegisterPtr("PopVector", &BSVM_JIT_PopVector);
	BASM_RegisterPtr("PopMatrix", &BSVM_JIT_PopMatrix);

	BASM_RegisterPtr("AddFl", &BSVM_JIT_AddFl);
	BASM_RegisterPtr("SubFl", &BSVM_JIT_SubFl);
	BASM_RegisterPtr("MulFl", &BSVM_JIT_MulFl);
	BASM_RegisterPtr("DivFl", &BSVM_JIT_DivFl);
	BASM_RegisterPtr("NegFl", &BSVM_JIT_NegFl);
	BASM_RegisterPtr("DivFn", &BSVM_JIT_DivFn);

	BASM_RegisterPtr("CmpEFl", &BSVM_JIT_CmpEFl);
	BASM_RegisterPtr("CmpNeFl", &BSVM_JIT_CmpNeFl);
	BASM_RegisterPtr("CmpLFl", &BSVM_JIT_CmpLFl);
	BASM_RegisterPtr("CmpGFl", &BSVM_JIT_CmpGFl);
	BASM_RegisterPtr("CmpLeFl", &BSVM_JIT_CmpLeFl);
	BASM_RegisterPtr("CmpGeFl", &BSVM_JIT_CmpGeFl);

	BASM_RegisterPtr("RetCleanup", &BSVM_JIT_RetCleanup);
...
#define BSVM_JIT_CACHE

#ifdef BSVM_JIT_CACHE

void BSVM_JIT_PushEAX_F(BSVM_JitContext *ctx)
{
//	basm_print("mov [edi+ebx*4], eax; inc_r ebx;\n");
	ctx->fl|=BSVM_JITFL_EAX;
}

void BSVM_JIT_PushEAX(BSVM_JitContext *ctx)
{
...
void BSVM_JIT_Push(BSVM_JitContext *ctx, elem val)
{
...
void BSVM_JIT_PopEAX_F(BSVM_JitContext *ctx)
{
	if(ctx->fl&BSVM_JITFL_EAX)
	{
		ctx->fl&=~BSVM_JITFL_EAX;
...
void BSVM_JIT_PopEAX(BSVM_JitContext *ctx)
{
//	ctx->fl&=~BSVM_JITFL_ECX_EARG;
...
void BSVM_JIT_PopToArg(BSVM_JitContext *ctx)
{
	ctx->fl&=~BSVM_JITFL_ECX_EARG;
...
void BSVM_JIT_PopToArgF(BSVM_JitContext *ctx)
{
	if(ctx->fl&BSVM_JITFL_EAX)
	{
		ctx->fl&=~BSVM_JITFL_EAX;
...
void BSVM_JIT_PopDestroy(BSVM_JitContext *ctx)
{
//	ctx->fl&=~BSVM_JITFL_ECX_EARG;
...
void BSVM_JIT_PopBool(BSVM_JitContext *ctx)
{
//	ctx->fl&=~BSVM_JITFL_ECX_EARG;
...
void BSVM_JIT_IncRef(BSVM_JitContext *ctx)
{
...
void BSVM_JIT_DecRef(BSVM_JitContext *ctx)
{
	if(ctx->fl&BSVM_JITFL_ECX_EARG)
	{
...
void BSVM_JIT_SafeDecRef(BSVM_JitContext *ctx)
{
	if(ctx->fl&BSVM_JITFL_ECX_EARG)
	{
...
void BSVM_JIT_FlattenStack(BSVM_JitContext *ctx)
{
	ctx->fl&=~BSVM_JITFL_ECX_EARG;
...
void BSVM_JIT_AdjustStack(BSVM_JitContext *ctx)
{
	BSVM_JIT_FlattenStack(ctx);
	basm_print("mov [esi+%d], ebx;\n", JIT_OFFS(stackpos));
}

void BSVM_JIT_Return(BSVM_JitContext *ctx)
{
	BSVM_JIT_AdjustStack(ctx);
	basm_print("push esi; call RetCleanup; pop ecx; pop ebx;\n"
		"pop edi; pop esi; pop ebp; ret;\n");
}

void BSVM_JIT_Entry(BSVM_JitContext *ctx)
{
...
void BSVM_JIT_Call(BSVM_JitContext *ctx)
{
	char *s;

	s=BSVM_JIT_GenSym(); BSVM_JIT_PopEAX(ctx); BSVM_JIT_AdjustStack(ctx);
	...
	basm_print("%s:\n", s); BSVM_JIT_Entry(ctx);
}

void BSVM_JIT_TailCall(BSVM_JitContext *ctx)
{
	char *s;
...
void BSVM_JIT_CallS(BSVM_JitContext *ctx, elem t)
{
	char *s;
...
void BSVM_JIT_TailCallS(BSVM_JitContext *ctx, elem t)
{
	char *s;
...
void BSVM_JIT_MethodCall(BSVM_JitContext *ctx)
{
	char *s;

	s=BSVM_JIT_GenSym(); BSVM_JIT_PopToArg(ctx);
	basm_print("mov dword [esi+%d], %s;\n", JIT_OFFS(thunk), s);
	BSVM_JIT_PopToArg(ctx); BSVM_JIT_AdjustStack(ctx);
	basm_print("push esi; call MethodCall; add esp, 12;\n");
	basm_print("xor eax, eax; pop ebx; pop edi; pop esi; pop ebp; ret;\n\n");
	basm_print("%s:\n", s); BSVM_JIT_Entry(ctx);
}

void BSVM_JIT_MethodTailCall(BSVM_JitContext *ctx)
{
	char *s;
...
void BSVM_JIT_MethodCallS(BSVM_JitContext *ctx, elem t)
{
	char *s;

	s=BSVM_JIT_GenSym(); basm_print("push dword 0x%x;\n", t);
	basm_print("mov dword [esi+%d], %s;\n", JIT_OFFS(thunk), s);
	BSVM_JIT_PopToArg(ctx); BSVM_JIT_AdjustStack(ctx);
	basm_print("push esi; call MethodCall; add esp, 12;\n");
	basm_print("xor eax, eax; pop ebx; pop edi; pop esi; pop ebp; ret;\n\n");
...
void BSVM_JIT_MethodTailCallS(BSVM_JitContext *ctx, elem t)
{
	char *s;
...
void BSVM_JIT_CallCF(BSVM_JitContext *ctx)
{
	char *s;

	s=BSVM_JIT_GenSym(); BSVM_JIT_AdjustStack(ctx);
	basm_print("mov dword [esi+%d], %s;\n", JIT_OFFS(thunk), s);
...
void BSVM_JIT_TailCallCF(BSVM_JitContext *ctx)
{
	char *s;
...
elem BSVM_JIT_ReadID(BSVM_JitContext *ctx)
{
	elem t;
	int i;
...
int BSVM_JIT_ReadIDX(BSVM_JitContext *ctx)
{
	int i;
	i=*ctx->ip++;
	if(i&128)i=((i&127)<<8)|(*ctx->ip++);
...
void BSVM_JIT_PreStep(BSVM_JitContext *ctx)
{
	char buf[64];
	byte *ip, *ip1;
	char *s;
...
char *BSVM_JIT_GetLabel(BSVM_JitContext *ctx, byte *ip)
{
	int i;
...
int BSVM_JIT_DumpOpcode(BSVM_JitContext *ctx)
{
	...
	while(*s)
	{
		if((*s=='I') || (*s=='S') || (*s=='T'))
		{
			...
			t=BSVM_TY_GetArrayElem2(ctx->lit, i);
//			BSVM_TY_Print(t);
			if(BSVM_SYMBOLP(t))basm_print("%s", BSVM_TOSYM(t));
			if(BSVM_KEYWORDP(t))basm_print("%s", BSVM_TOSYM(t));
			if(BSVM_STRINGP(t))basm_print("'%s'", BSVM_TOSTRING(t));
			basm_print(" ");
			s++;
			continue;
		}
...
int BSVM_JIT_Step(BSVM_JitContext *ctx)
{
	elem t, u, v;
	int i, j, rs;
	char *s, *s1, *s2;
	...
	case BSVM_SOP_PUSH:
		t=BSVM_JIT_ReadID(ctx);
		BSVM_JIT_Push(ctx, t);
		break;
	case BSVM_SOP_POP:
		BSVM_JIT_PopDestroy(ctx);
		break;
	case BSVM_SOP_LOAD:
		BSVM_JIT_FlattenStack(ctx);
		t=BSVM_JIT_ReadID(ctx);
		basm_print("push dword 0x%x; push esi; call Lookup; add esp, 8;\n", t);
		BSVM_JIT_PushEAX(ctx);
		break;
	case BSVM_SOP_STORE:
		t=BSVM_JIT_ReadID(ctx);
		BSVM_JIT_PopToArgF(ctx);
	...
	case BSVM_SOP_PUSH_NULL:
		BSVM_JIT_Push(ctx, BSVM_MM_NULL);
		break;
	case BSVM_SOP_PUSH_TRUE:
		BSVM_JIT_Push(ctx, BSVM_MM_TRUE);
		break;
	case BSVM_SOP_PUSH_FALSE:
		BSVM_JIT_Push(ctx, BSVM_MM_FALSE);
		break;
	case BSVM_SOP_PUSH_SV:
		i=(signed char)(*ctx->ip++);
		BSVM_JIT_Push(ctx, FIXNUM(i));
		break;
	...
	case BSVM_SOP_PUSH_SV_F:
		i=*ctx->ip++;
		i+=(*ctx->ip++)<<8;
		BSVM_JIT_Push(ctx, FLONUM(((short)i)/100.0));
		break;

	case BSVM_SOP_PUSH_0: BSVM_JIT_Push(ctx, BSVM_MM_FIXNUM_0); break;
	case BSVM_SOP_PUSH_1: BSVM_JIT_Push(ctx, BSVM_MM_FIXNUM_1); break;
	case BSVM_SOP_PUSH_2: BSVM_JIT_Push(ctx, BSVM_MM_FIXNUM_2); break;
	case BSVM_SOP_PUSH_3: BSVM_JIT_Push(ctx, BSVM_MM_FIXNUM_3); break;
	case BSVM_SOP_PUSH_4: BSVM_JIT_Push(ctx, BSVM_MM_FIXNUM_4); break;
	case BSVM_SOP_PUSH_5: BSVM_JIT_Push(ctx, BSVM_MM_FIXNUM_5); break;
	case BSVM_SOP_PUSH_6: BSVM_JIT_Push(ctx, BSVM_MM_FIXNUM_6); break;
	case BSVM_SOP_PUSH_7: BSVM_JIT_Push(ctx, BSVM_MM_FIXNUM_7); break;
	case BSVM_SOP_PUSH_CF: BSVM_JIT_Push(ctx, ctx->func); break;
...
//		BSVM_JIT_FlattenStack(ctx);
	...
	case BSVM_SOP_CALL:	BSVM_JIT_Call(ctx); break;
	case BSVM_SOP_TAILCALL:	BSVM_JIT_TailCall(ctx); break;
	case BSVM_SOP_CALL_S:
		t=BSVM_JIT_ReadID(ctx); BSVM_JIT_CallS(ctx, t); break;
	case BSVM_SOP_TAILCALL_S:
		t=BSVM_JIT_ReadID(ctx); BSVM_JIT_TailCallS(ctx, t); break;

	case BSVM_SOP_METHODCALL:	BSVM_JIT_MethodCall(ctx); break;
	case BSVM_SOP_METHODTAILCALL:	BSVM_JIT_MethodTailCall(ctx); break;
	case BSVM_SOP_METHODCALL_S:
		t=BSVM_JIT_ReadID(ctx); BSVM_JIT_MethodCallS(ctx, t); break;
	case BSVM_SOP_METHODTAILCALL_S:
		t=BSVM_JIT_ReadID(ctx); BSVM_JIT_MethodTailCallS(ctx, t); break;

	case BSVM_SOP_CALL_CF:		BSVM_JIT_CallCF(ctx); break;
	case BSVM_SOP_TAILCALL_CF:	BSVM_JIT_TailCallCF(ctx); break;

	case BSVM_SOP_RET:	BSVM_JIT_Return(ctx); break;
...
//		BSVM_JIT_FlattenStack(ctx);
	...
	case BSVM_SOP_JMP:
		...
		s=BSVM_JIT_GetLabel(ctx, ctx->ip+i);
		basm_print("jmp %s;\n", s);
		ctx->fl&=~BSVM_JITFL_ECX_EARG;
		break;
...
//		BSVM_JIT_FlattenStack(ctx);
		...
		s=BSVM_JIT_GetLabel(ctx, ctx->ip+i);
		s1=BSVM_JIT_GenSym(); s2=BSVM_JIT_GenSym();
...
//		j=BSVM_Opr_CondBinary(j, u, v);
	...
	case BSVM_SOP_CMP_L_FN:
		BSVM_JIT_FlattenStack(ctx);
		s=BSVM_JIT_GenSym(); s1=BSVM_JIT_GenSym();
		basm_print("dec_r ebx; mov eax, [edi+ebx*4-4]; mov ecx, [edi+ebx*4];\n"
	...
	case BSVM_SOP_JMP_L_FN:
		BSVM_JIT_FlattenStack(ctx);
		i=*ctx->ip++; i|=(*ctx->ip++)<<8;
		s=BSVM_JIT_GetLabel(ctx, ctx->ip+((signed short)i));
...
void BSVM_JIT_CompileBlock(elem blk)
{
	BSVM_JitContext *ctx;
	BSVM_SVMBlock *fcn;
	byte *ip;
	...
	SET(fcn->th_name, SYM(BSVM_JIT_GenSym()));
