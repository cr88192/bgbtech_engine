...
//	BGBCC_StoreFile(name, ctx->img_base, ctx->img_size);
	vf_storefile(name, ctx->img_base, ctx->img_size);
}
...
//	i=BGBCC_StoreFile(ctx->name, ctx->img_base, sz);
	i=vf_storefile(ctx->name, ctx->img_base, sz);
	if(i<0)
	{
		dyPrintf("ExWAD_SaveImage: failed write %s\n", ctx->name);
		return(-1);
	}

	dyPrintf("ExWAD_SaveImage: wrote %s, %d bytes\n", ctx->name, sz);
	return(0);
}
...
//	if(!buf)return(NULL);
	...
	if((buf[0]=='M') && (buf[1]=='Z'))
	{
		...
		{
			buf1=buf+i;
//			if(!strncmp(buf1, "ExWAD100", 8))
//			if(!memcmp(buf1, "ExWAD100", 8))
...
//			if(!strncmp(buf1, "ExWAD100", 8))
//			if(!memcmp(buf1, "ExWAD100", 8) &&
//				!memcmp(buf1+EXWAD_CELLSZ, "DAWxE\r\n\xFF", 8))
			if(!memcmp(buf1, EXWAD_MAGIC1, 8) &&
	...
	if(tmp->head)
	{
		...
		ExWAD_MarkRangeResv(tmp, 0, tmp->head_rva+EXWAD_CELLSZ);
...
void ExWAD_CheckExpandImage(ExWAD_Context *img, int sz)
{
	...
	dyPrintf("ExWAD_CheckExpandImage: %d\n", sz);
...
//		return;	//not PE/COFF
	...
	dyPrintf("ExWAD_FixupEXE: SectionAlignment=%d, SizeOfImage=%d\n",
		pealgn, peszimg);
...
//	exwad_setu32(ctx->img_base+pehdr+24+56, i*pealgn);
	...
	dyPrintf("ExWAD_FixupEXE: new SizeOfImage=%d\n",
		ctx->img_saveSize);
...
void ExWAD_FixupImage(ExWAD_Context *ctx)
{
	...
	dyPrintf("ExWAD_FixupImage: End of Image At %08X, %d\n",
...
BGBDY_API ExWAD_Context *ExWAD_LoadImage2(char *name)
{
	...
	dyPrintf("ExWAD_LoadImage2: Create header\n");
	
	i=ExWAD_AllocRange(tmp, EXWAD_CELLSZ);
	tmp->head=(ExWAD_Header *)(tmp->img_base+(i*EXWAD_CELLSZ));
	tmp->head_rva=i*EXWAD_CELLSZ;

	dyPrintf("ExWAD_LoadImage2: Create header at %08X\n", tmp->head_rva);

//	memcpy(tmp->head->magic, "ExWAD100", 8);
//	memcpy(tmp->head->magic2, "DAWxE\r\n\xFF", 8);

	memcpy(tmp->head->magic, EXWAD_MAGIC1, 8);
	memcpy(tmp->head->magic2, EXWAD_MAGIC2, 8);

	i=ExWAD_AllocRange(tmp, EXWAD_CELLSZ*sizeof(ExWAD_DirEnt));
	tmp->dir=(ExWAD_DirEnt *)(tmp->img_base+(i*EXWAD_CELLSZ));
	exwad_setu32(tmp->head->rva_dirents, i*EXWAD_CELLSZ);
	exwad_setu32(tmp->head->num_dirents, EXWAD_CELLSZ);

	dyPrintf("ExWAD_LoadImage2: Create dir at %08X..%08X\n",
		i*EXWAD_CELLSZ, i*EXWAD_CELLSZ+16*sizeof(ExWAD_DirEnt));
...
BGBDY_API ExWAD_Context *ExWAD_LoadImageBufferRead(
	...
	int idx, char *nbuf)
{
	...
	if(fl&EXWAD_DIRFL_RVANAME)
	{
		...
		{
//			i=exwad_getu32(de->name+(EXWAD_NAMESZ-4));
...
//		i=exwad_getu16(de->name+(EXWAD_NAMESZ-2));
		i=exwad_getu32(de->dir);
		ExWAD_GetDirEntIndexName(ctx, i, tb);
		s=tb+strlen(tb);
...
void ExWAD_CheckExpandDir(ExWAD_Context *ctx, int num)
{
	...
	dyPrintf("ExWAD_CheckExpandDir: %d\n", num);
...
BGBDY_API byte *ExWAD_LoadFile(
	ExWAD_Context *ctx, char *name, int *rsz)
{
	ExWAD_DirEnt *de;
	byte *buf;
	int i, j, k, l, b, sz;
...
BGBDY_API void ExWAD_StoreFile(
	ExWAD_Context *ctx, char *name, byte *buf, int sz)
{
	ExWAD_DirEnt *de;
	int i, j, k, l, b, di;
	...
	if(j||k)
	{
		dyPrintf("ExWAD_StoreFile: free range, %08X..%08X, %d bytes\n",
			j, j+k, k);
		ExWAD_MarkRangeFree(ctx, j, k);
	}

	i=ExWAD_AllocRange(ctx, sz);
	b=i*EXWAD_CELLSZ;

	dyPrintf("ExWAD_StoreFile: Using Range, %08X..%08X, %d bytes\n",
		b, b+sz, sz);

	de=&(ctx->dir[di]);		//dir or image may have moved

	memcpy(ctx->img_base+b, buf, sz);
	exwad_setu32(de->rva_data, b);
	exwad_setu32(de->csize_data, sz);
	exwad_setu32(de->dsize_data, sz);
	de->method=EXWAD_DIR_CM_STORE;
}

#if 0
// BGBDY_API void ExWAD_AddFile(ExWAD_Context *ctx, char *name)
{
	byte *buf, *buf1;
	int i, j, k, n, sz;
...
BGBDY_API ExWAD_CachedFile *ExWAD_LoadCachedFile(
	ExWAD_Context *ctx, char *name)
{
	ExWAD_CachedFile *cur;
	byte *buf;
	int sz;
	...
	buf=ExWAD_LoadFile(ctx, name, &sz);
	if(!buf)return(NULL);
	
	cur=malloc(sizeof(ExWAD_CachedFile));
	memset(cur, 0, sizeof(ExWAD_CachedFile));
...
BGBDY_API ExWAD_File *ExWAD_OpenFile(
	ExWAD_Context *ctx, char *name)
{
	ExWAD_CachedFile *ent;
	ExWAD_File *tmp;
	
	ent=ExWAD_LoadCachedFile(ctx, name);
	if(!ent)return(NULL);
	
	tmp=gcalloc(sizeof(ExWAD_File));
	tmp->ctx=ctx;
	tmp->ent=ent;
	tmp->offs=0;
