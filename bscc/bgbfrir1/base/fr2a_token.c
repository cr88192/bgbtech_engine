#include <bgbfrir2.h>

char *bgbfr2_asmparse_pbase;
char *bgbfr2_asmparse_fname;
int bgbfr2_asmparse_linenum=1;

char *bgbfr2_asmparse_pbasestack[16];
char *bgbfr2_asmparse_fnamestack[16];
int bgbfr2_asmparse_lnumstack[16];
int bgbfr2_asmparse_lnumstackpos;

extern char *bgbfr2_asmparse_curfilename;
extern int bgbfr2_asmparse_curlinenum;

BGBFR2_API int BGBFR2_AsmParse_GetLinenum()
	{ return(bgbfr2_asmparse_linenum); }

BGBFR2_API char *BGBFR2_AsmParse_GetFilename()
	{ return(bgbfr2_asmparse_fname); }

int BGBFR2_AsmParse_CalcLinenum(char *se)
{
	char *s;
	int i;

	if(!bgbfr2_asmparse_pbase)
		return(0);

	s=bgbfr2_asmparse_pbase;
	i=bgbfr2_asmparse_linenum;
	while(s<se)
	{
		if(*s=='\n')i++;
		if((s[0]=='\r') && (s[1]!='\n'))i++;
		s++;
	}

	return(i);
}

BGBFR2_API int BGBFR2_AsmParse_SetLinenum(char *fname, char *base, int num)
{
	bgbfr2_asmparse_curfilename=NULL;
	bgbfr2_asmparse_curlinenum=-1;

	bgbfr2_asmparse_fname=fname;
	bgbfr2_asmparse_pbase=base;
	bgbfr2_asmparse_linenum=num;
	return(num);
}

int BGBFR2_AsmParse_PushLinenum()
{
	bgbfr2_asmparse_curfilename=NULL;
	bgbfr2_asmparse_curlinenum=-1;

	bgbfr2_asmparse_lnumstack[bgbfr2_asmparse_lnumstackpos]=
		bgbfr2_asmparse_linenum;
	bgbfr2_asmparse_pbasestack[bgbfr2_asmparse_lnumstackpos]=
		bgbfr2_asmparse_pbase;
	bgbfr2_asmparse_fnamestack[bgbfr2_asmparse_lnumstackpos]=
		bgbfr2_asmparse_fname;
	bgbfr2_asmparse_lnumstackpos++;
	return(0);
}

int BGBFR2_AsmParse_PopLinenum()
{
	bgbfr2_asmparse_lnumstackpos--;
	bgbfr2_asmparse_linenum=
		bgbfr2_asmparse_lnumstack[bgbfr2_asmparse_lnumstackpos];
	bgbfr2_asmparse_pbase=
		bgbfr2_asmparse_pbasestack[bgbfr2_asmparse_lnumstackpos];
	bgbfr2_asmparse_fname=
		bgbfr2_asmparse_fnamestack[bgbfr2_asmparse_lnumstackpos];
	return(bgbfr2_asmparse_linenum);
}

static int BSVM_MSG_Print(char *se, char *ty, char *s, va_list lst)
{
	char buf[256];
	char *t;

	t=buf;
	if(bgbfr2_asmparse_fname)
	{
		sprintf(t, "%s: ", bgbfr2_asmparse_fname);
		t+=strlen(t);
	}
	sprintf(t, "%s: %d: ", ty, BGBFR2_AsmParse_CalcLinenum(se));
	t+=strlen(t);

	vsprintf(t, s, lst);

	printf("%s", buf);
//	kprint_logstr(buf);
	return(0);
}

int BSVM_MSG_Note(char *se, char *s, ...)
{
	va_list lst;

	va_start(lst, s);
	BSVM_MSG_Print(se, "note", s, lst);
	va_end(lst);
	return(0);
}

int BSVM_MSG_Warning(char *se, char *s, ...)
{
	va_list lst;

	va_start(lst, s);
	BSVM_MSG_Print(se, "warning", s, lst);
	va_end(lst);
	return(0);
}

int BSVM_MSG_Error(char *se, char *s, ...)
{
	va_list lst;

	va_start(lst, s);
	BSVM_MSG_Print(se, "error", s, lst);
	va_end(lst);

//	*(int *)-1=-1;

	return(0);
}

char *BGBFR2_AsmParse_EatWhiteOnly(char *s)
{
	if(!s)return(s);
	while(*s && (*s<=' '))s++;
	return(s);
}

char *BGBFR2_AsmParse_EatWhite(char *s)
{
	int i;

	if(!s)return(s);
	while(*s && (*s<=' '))s++;

	if((s[0]=='/') && (s[1]=='/'))
	{
		while(*s && (*s!='\n'))s++;
		s=BGBFR2_AsmParse_EatWhite(s);
		return(s);
	}
	if((s[0]=='/') && (s[1]=='*'))
	{
		s+=2;
		i=1;
		while(*s && i)
		{
			if((s[0]=='/') && (s[1]=='*'))
			{
				s+=2;
				i++;
				continue;
			}
			if((s[0]=='*') && (s[1]=='/'))
			{
				s+=2;
				i--;
				continue;
			}
			s++;
		}
		s=BGBFR2_AsmParse_EatWhite(s);
		return(s);
	}

	return(s);
}

int BGBFR2_AsmParse_IsWhite(char *s)
{
	if(!s)return(0);
	if(*s && (*s<=' '))
		return(1);
	return(0);
}

int BGBFR2_AsmParse_IsWhiteOnly(char *s)
{
	if(!s)return(0);

	while((*s==' ') || (*s=='\t'))
		s++;

	if((s[0]=='/') && (s[1]=='/'))return(0);
	if((s[0]=='/') && (s[1]=='*'))return(0);
	if((s[0]=='*') && (s[1]=='/'))return(0);

	if(*s>' ')return(1);
	if(*s==' ')return(1);
	if(*s=='\t')return(1);

	return(0);
}

int BGBFR2_AsmParse_IsNextIndented(char *se)
{
	char *s, *b, *t;
	int i, j;

	if(!bgbfr2_asmparse_pbase)
		return(0);

	s=bgbfr2_asmparse_pbase; b=NULL;
	if(s>se)return(0);
	
	while(s<se)
	{
		if(!*s)return(0);

		if(*s=='\n')b=s+1;
		if((s[0]=='\r') && (s[1]!='\n'))b=s+2;
		s++;
	}

	if(!b)return(0);	//no next line

	t=b; i=0;
	while(*t && (*t<=' '))
	{
		if(*t=='\t')i+=8;
			else i++;
		t++;
	}

	while(*s)
	{
		if(*s=='\n') { b=s+1; break; }
		if((s[0]=='\r') && (s[1]!='\n')) { b=s+2; break; }
		s++;
	}

	t=b; j=0;
	while(*t && (*t<=' '))
	{
		if(*t=='\t')j+=8;
			else j++;
		t++;
	}

	return(j>i);
}

int BGBFR2_AsmParse_IsLineBreak(char *se)
{
	char *s, *b, *t;
	int i, j;

	s=se;
	while(*s && (*s<=' '))
	{
		if(*s=='\r')return(1);
		if(*s=='\n')return(1);
		s++;
	}
	if(!*s)return(1);

	return(0);
}


int BGBFR2_AsmParse_OpChar(int c)
{
	int i;

	switch(c)
	{
	case '~':
	case '!':
	case '@':
	case '#':
	case '$':
	case '%':
	case '^':
	case '&':
	case '*':
	case '-':
	case '=':
	case '+':

	case '\\':
	case '|':

	case ':':

	case '<':
	case '>':
	case '?':
	case '/':

	case '.':
		i=1;
		break;

	default:
		i=0;
		break;
	}
	return(i);
}

int BGBFR2_AsmParse_PeekChar(char **str)
{
	unsigned char *s;
	int i;

	s=(unsigned char *)(*str);
	i=*s++;

	if(!(i&0x80))return(i);

	if(i>=0xFC)
	{
		i=(i&1)<<30;
		i+=((*s++)&63)<<24;
		i+=((*s++)&63)<<18;
		i+=((*s++)&63)<<12;
		i+=((*s++)&63)<<6;
		i+=(*s++)&63;
	}else if(i>=0xF8)
	{
		i=(i&3)<<24;
		i+=((*s++)&63)<<18;
		i+=((*s++)&63)<<12;
		i+=((*s++)&63)<<6;
		i+=(*s++)&63;
	}else if(i>=0xF0)
	{
		i=(i&7)<<18;
		i+=((*s++)&63)<<12;
		i+=((*s++)&63)<<6;
		i+=(*s++)&63;
	}else if(i>=0xE0)
	{
		i=(i&15)<<12;
		i+=((*s++)&63)<<6;
		i+=(*s++)&63;
	}else if(i>=0xC0)
	{
		i=(i&31)<<6;
		i+=(*s++)&63;
	}else if(i>=0x80)
	{
//		continue;
	}

	return(i);
}

int BGBFR2_AsmParse_ParseChar(char **str)
{
	unsigned char *s;
	int i;

	s=(unsigned char *)(*str);
	i=*s++;

	if(!(i&0x80))
	{
		*str=(char *)s;
		return(i);
	}

	if(i>=0xFC)
	{
		i=(i&1)<<30;
		i+=((*s++)&63)<<24;
		i+=((*s++)&63)<<18;
		i+=((*s++)&63)<<12;
		i+=((*s++)&63)<<6;
		i+=(*s++)&63;
	}else if(i>=0xF8)
	{
		i=(i&3)<<24;
		i+=((*s++)&63)<<18;
		i+=((*s++)&63)<<12;
		i+=((*s++)&63)<<6;
		i+=(*s++)&63;
	}else if(i>=0xF0)
	{
		i=(i&7)<<18;
		i+=((*s++)&63)<<12;
		i+=((*s++)&63)<<6;
		i+=(*s++)&63;
	}else if(i>=0xE0)
	{
		i=(i&15)<<12;
		i+=((*s++)&63)<<6;
		i+=(*s++)&63;
	}else if(i>=0xC0)
	{
		i=(i&31)<<6;
		i+=(*s++)&63;
	}else if(i>=0x80)
	{
//		continue;
	}

	*str=(char *)s;
	return(i);
}

int BGBFR2_AsmParse_EmitChar(char **str, int j)
{
	unsigned char *t;
	int i;

	t=(unsigned char *)(*str);

	if(j<128)
	{
		if(j<=0) //Modified-UTF8
			{*t++=0xC0; *t++=0x80;}
			else *t++=j;
		*str=(char *)t;
		return(0);
	}

	if(j>0x3FFFFFF)
	{
		*t++=0xFC+(j>>30);
		*t++=0x80+((j>>24)&0x3F);
		*t++=0x80+((j>>18)&0x3F);
		*t++=0x80+((j>>12)&0x3F);
		*t++=0x80+((j>>6)&0x3F);
		*t++=0x80+(j&0x3F);
	}else if(j>0x1FFFFF)
	{
		*t++=0xF8+(j>>24);
		*t++=0x80+((j>>18)&0x3F);
		*t++=0x80+((j>>12)&0x3F);
		*t++=0x80+((j>>6)&0x3F);
		*t++=0x80+(j&0x3F);
	}else if(j>0xFFFF)
	{
		*t++=0xF0+(j>>18);
		*t++=0x80+((j>>12)&0x3F);
		*t++=0x80+((j>>6)&0x3F);
		*t++=0x80+(j&0x3F);
	}else if(j>0x7FF)
	{
		*t++=0xE0+(j>>12);
		*t++=0x80+((j>>6)&0x3F);
		*t++=0x80+(j&0x3F);
	}else if((j>0x7F) || !j)
	{
		*t++=0xC0+(j>>6);
		*t++=0x80+(j&0x3F);
	}else
	{
		if(!j) //Modified-UTF8
			{ *t++=0xC0; *t++=0x80; }
			else *t++=j;
	}

//	}else *t++=j;

	*str=(char *)t;
	return(0);
}

int BGBFR2_AsmParse_NameInitChar(int c)
{
	static int vals[]={
	0x00AA, 0x00BA, 0x207F, 0x0386, 0x038C, 0x03DA, 0x03DC, 0x03DE,
	0x03E0, 0x1F59, 0x1F5B, 0x1F5D, 0x05BF, 0x09B2, 0x0A02, 0x0A5E,
	0x0A74, 0x0A8D, 0x0AD0, 0x0AE0,
	0x0B9C, 0x0CDE, 0x0E84, 0x0E8A, 0x0E8D, 0x0EA5, 0x0EA7, 0x0EC6,
	0x0F00, 0x0F35, 0x0F37, 0x0F39, 0x0F97, 0x0FB9, 0x00B5, 0x00B7,
	0x02BB, 0x037A, 0x0559, 0x093D, 0x0B3D, 0x1FBE, 0x2102, 0x2107,
	0x2115, 0x2124,	0x2126, 0x2128, 0}; 

	static int rngs[]={
	0x0388, 0x038A, 0x038E, 0x03A1, 0x03A3, 0x03CE, 0x03D0, 0x03D6,
	0x03E2, 0x03F3, 0x1F00, 0x1F15, 0x1F18, 0x1F1D, 0x1F20, 0x1F45,
	0x1F48, 0x1F4D, 0x1F50, 0x1F57, 0x1F5F, 0x1F7D, 0x1F80, 0x1FB4,
	0x1FB6, 0x1FBC, 0x1FC2, 0x1FC4, 0x1FC6, 0x1FCC, 0x1FD0, 0x1FD3,
	0x1FD6, 0x1FDB, 0x1FE0, 0x1FEC, 0x1FF2, 0x1FF4, 0x1FF6, 0x1FFC,
	0x0401, 0x040C, 0x040E, 0x044F, 0x0451, 0x045C, 0x045E, 0x0481,
	0x0490, 0x04C4, 0x04C7, 0x04C8, 0x04CB, 0x04CC, 0x04D0, 0x04EB,
	0x04EE, 0x04F5, 0x04F8, 0x04F9, 0x0531, 0x0556, 0x0561, 0x0587,
	0x05B0, 0x05B9, 0x05BB, 0x05BD, 0x05C1, 0x05C2, 0x05D0, 0x05EA,
	0x05F0, 0x05F2, 0x0621, 0x063A, 0x0640, 0x0652, 0x0670, 0x06B7,
	0x06BA, 0x06BE, 0x06C0, 0x06CE, 0x06D0, 0x06DC, 0x06E5, 0x06E8,
	0x06EA, 0x06ED, 0x0901, 0x0903, 0x0905, 0x0939, 0x093E, 0x094D,
	0x0950, 0x0952, 0x0958, 0x0963, 0x0981, 0x0983, 0x0985, 0x098C,
	0x098F, 0x0990, 0x0993, 0x09A8, 0x09AA, 0x09B0, 0x09B6, 0x09B9,
	0x09BE, 0x09C4, 0x09C7, 0x09C8, 0x09CB, 0x09CD, 0x09DC, 0x09DD,
	0x09DF, 0x09E3, 0x09F0, 0x09F1, 0x0A05, 0x0A0A, 0x0A0F, 0x0A10,
	0x0A13, 0x0A28, 0x0A2A, 0x0A30, 0x0A32, 0x0A33, 0x0A35, 0x0A36,
	0x0A38, 0x0A39, 0x0A3E, 0x0A42, 0x0A47, 0x0A48, 0x0A4B, 0x0A4D,
	0x0A59, 0x0A5C, 0x0A81, 0x0A83, 0x0A85, 0x0A8B, 0x0A8F, 0x0A91,
	0x0A93, 0x0AA8, 0x0AC7, 0x0AC9, 0x0ACB, 0x0ACD, 0x00C0, 0x00D6,
	0x00D8, 0x00F6, 0x00F8, 0x01F5, 0x01FA, 0x0217, 0x0250, 0x02A8,
	0x1E00, 0x1E9B, 0x1EA0, 0x1EF9, 0x0AAA, 0x0AB0, 0x0AB2, 0x0AB3,
	0x0AB5, 0x0AB9, 0x0ABD, 0x0AC5,
	0x0B01, 0x0B03, 0x0B05, 0x0B0C, 0x0B0F, 0x0B10, 0x0B13, 0x0B28,
	0x0B2A, 0x0B30, 0x0B32, 0x0B33, 0x0B36, 0x0B39, 0x0B3E, 0x0B43,
	0x0B47, 0x0B48, 0x0B4B, 0x0B4D, 0x0B5C, 0x0B5D, 0x0B5F, 0x0B61,
	0x0B82, 0x0B83, 0x0B85, 0x0B8A, 0x0B8E, 0x0B90, 0x0B92, 0x0B95,
	0x0B99, 0x0B9A, 0x0B9E, 0x0B9F, 0x0BA3, 0x0BA4, 0x0BA8, 0x0BAA,
	0x0BAE, 0x0BB5, 0x0BB7, 0x0BB9, 0x0BBE, 0x0BC2, 0x0BC6, 0x0BC8,
	0x0BCA, 0x0BCD, 0x0C01, 0x0C03, 0x0C05, 0x0C0C, 0x0C0E, 0x0C10,
	0x0C12, 0x0C28, 0x0C2A, 0x0C33, 0x0C35, 0x0C39, 0x0C3E, 0x0C44,
	0x0C46, 0x0C48, 0x0C4A, 0x0C4D, 0x0C60, 0x0C61, 0x0C82, 0x0C83,
	0x0C85, 0x0C8C, 0x0C8E, 0x0C90, 0x0C92, 0x0CA8, 0x0CAA, 0x0CB3,
	0x0CB5, 0x0CB9, 0x0CBE, 0x0CC4, 0x0CC6, 0x0CC8, 0x0CCA, 0x0CCD,
	0x0CE0, 0x0CE1, 0x0D02, 0x0D03, 0x0D05, 0x0D0C, 0x0D0E, 0x0D10,
	0x0D12, 0x0D28, 0x0D2A, 0x0D39, 0x0D3E, 0x0D43, 0x0D46, 0x0D48,
	0x0D4A, 0x0D4D, 0x0D60, 0x0D61, 0x0E01, 0x0E3A, 0x0E40, 0x0E5B,
	0x0E81, 0x0E82, 0x0E87, 0x0E88, 0x0E94, 0x0E97, 0x0E99, 0x0E9F,
	0x0EA1, 0x0EA3, 0x0EAA, 0x0EAB, 0x0EAD, 0x0EAE, 0x0EB0, 0x0EB9,
	0x0EBB, 0x0EBD, 0x0EC0, 0x0EC4, 0x0EC8, 0x0ECD, 0x0EDC, 0x0EDD,
	0x0F18, 0x0F19, 0x0F3E, 0x0F47, 0x0F49, 0x0F69, 0x0F71, 0x0F84,
	0x0F86, 0x0F8B, 0x0F90, 0x0F95, 0x0F99, 0x0FAD, 0x0FB1, 0x0FB7,
	0x10A0, 0x10C5, 0x10D0, 0x10F6, 0x3041, 0x3093, 0x309B, 0x309C,
	0x30A1, 0x30F6, 0x30FB, 0x30FC, 0x3105, 0x312C, 0x4E00, 0x9FA5,
	0xAC00, 0xD7A3, 0x0660, 0x0669, 0x06F0, 0x06F9, 0x0966, 0x096F,
	0x09E6, 0x09EF, 0x0A66, 0x0A6F, 0x0AE6, 0x0AEF, 0x0B66, 0x0B6F,
	0x0BE7, 0x0BEF, 0x0C66, 0x0C6F, 0x0CE6, 0x0CEF, 0x0D66, 0x0D6F,
	0x0E50, 0x0E59, 0x0ED0, 0x0ED9, 0x0F20, 0x0F33, 0x02B0, 0x02B8,
	0x02BD, 0x02C1, 0x02D0, 0x02D1, 0x02E0, 0x02E4, 0x203F, 0x2040,
	0x210A, 0x2113, 0x2118, 0x211D, 0x212A, 0x2131, 0x2133, 0x2138,
	0x2160, 0x2182, 0x3005, 0x3007, 0x3021, 0x3029, 0};

	int i;

	if((c>='a') && (c<='z'))return(1);
	if((c>='A') && (c<='Z'))return(1);
	if(c=='_')return(1);
	if(c=='$')return(1);

//	if((c>='0') && (c<='9'))return(0);
	if(c<128)return(0);

	for(i=0; vals[i]; i++)
		if(c==vals[i])return(1);

	for(i=0; rngs[i*2+0]; i++)
		if((c>=rngs[i*2+0]) && (c<=rngs[i*2+1]))
			return(1);

	return(0);
}

int BGBFR2_AsmParse_NameChar(int c)
{
	if((c>='0') && (c<='9'))return(1);
	if(BGBFR2_AsmParse_NameInitChar(c))return(1);
	return(0);
}

char *BGBFR2_AsmParse_Token(char *s, char *b, int *ty)
{
	char *t, *s1, *te;
	int i, j, k;

	if(!s)
	{
		*ty=BGBFR2_TOKEN_NULL;
		return(s);
	}
	
	if(!b)b=gcralloc(256);

	*b=0;
	s=BGBFR2_AsmParse_EatWhite(s);
	if(!*s)
	{
		*ty=BGBFR2_TOKEN_NULL;
		return(s);
	}
	t=b; te=t+256-2;

#if 0
	if((*s=='_') || ((*s>='a') && (*s<='z')) || ((*s>='A') && (*s<='Z')))
	{
		while(
			(*s=='_') ||
			((*s>='a') && (*s<='z')) ||
			((*s>='A') && (*s<='Z')) ||
			((*s>='0') && (*s<='9')))
			*t++=*s++;
		*t++=0;

		*ty=BGBFR2_TOKEN_NAME;
		return(s);
	}
#endif

#if 1
	i=BGBFR2_AsmParse_PeekChar(&s);
	if(BGBFR2_AsmParse_NameInitChar(i))
	{
		while(t<te)
		{
#if 1
			if((s[0]=='\\') && ((s[1]=='u') || (s[1]=='U')))
			{
				k=2;
				if(s[1]=='u')k=4;
				if(s[1]=='U')k=8;

				s+=2; j=0;
				while(k--)
				{
					j<<=4;
					if((*s>='0') && (*s<='9'))j+=*s-'0';
					if((*s>='A') && (*s<='F'))j+=*s-'A'+10;
					if((*s>='a') && (*s<='f'))j+=*s-'a'+10;
					s++;
				}
				BGBFR2_AsmParse_EmitChar(&t, j);
				continue;
			}
#endif

			s1=s;
			i=BGBFR2_AsmParse_ParseChar(&s);
			if(!BGBFR2_AsmParse_NameChar(i))
				{ s=s1; break; }
			BGBFR2_AsmParse_EmitChar(&t, i);
		}

		*t++=0;

		*ty=BGBFR2_TOKEN_NAME;
		return(s);
	}
#endif

	if((*s>='0') && (*s<='9'))
	{
		if((s[0]=='0') && (s[1]=='x'))
		{
			*t++=*s++;
			*t++=*s++;
			while(((*s>='0') && (*s<='9')) ||
				((*s>='A') && (*s<='F')) ||
				((*s>='a') && (*s<='f')) ||
				(*s=='_'))
			{
				if(t>=te)break;
				*t++=*s++;
			}
			*t++=0;

			*ty=BGBFR2_TOKEN_NUMBER;
			return(s);
		}

		if((s[0]=='0') && ((s[1]=='o') ||
			(s[1]=='b') || (s[1]=='c') || (s[1]=='d')))
		{
			*t++=*s++;
			*t++=*s++;
			while(((*s>='0') && (*s<='9')) || (*s=='_'))
			{
				if(t>=te)break;
				*t++=*s++;
			}
			*t++=0;

			*ty=BGBFR2_TOKEN_NUMBER;
			return(s);
		}

#if 0
		if(*s=='-')*t++=*s++;
		while(((*s>='0') && (*s<='9')) || (*s=='.') || (*s=='e'))
		{
			if(*s=='e')
			{
				*t++=*s++;
				if((*s=='+') || (*s=='-'))
					*t++=*s++;
				continue;
			}
			*t++=*s++;
		}
		*t++=0;
#endif

#if 1
		while(((*s>='0') && (*s<='9')) || (*s=='_'))
		{
			if(t>=te)break;
			*t++=*s++;
		}
		if((*s=='.') && (s[1]>='0') && (s[1]<='9') && (t<te))
		{
			*t++=*s++;
			while((*s>='0') && (*s<='9') && (t<te))
				*t++=*s++;
			if(*s=='e')
			{
				*t++=*s++;
				if((*s=='+') || (*s=='-'))*t++=*s++;
				while((*s>='0') && (*s<='9') && (t<te))
					*t++=*s++;
			}
		}
		*t++=0;
#endif

		*ty=BGBFR2_TOKEN_NUMBER;
		return(s);
	}

	if((*s=='"') || (*s=='\'')) /* quoted string */
	{
		if(*s=='\'')i=1;
			else i=0;
		s++;
		while(*s && (t<te))
		{
			if(!i)if(*s=='"')break;
			if(i)if(*s=='\'')break;

			if((s[0]=='\\') &&
				((s[1]=='x') || (s[1]=='u') || (s[1]=='U')))
			{
				k=2;
				if(s[1]=='x')k=2;
				if(s[1]=='u')k=4;
				if(s[1]=='U')k=8;

				s+=2; j=0;
				while(k--)
				{
					j<<=4;
					if((*s>='0') && (*s<='9'))j+=*s-'0';
					if((*s>='A') && (*s<='F'))j+=*s-'A'+10;
					if((*s>='a') && (*s<='f'))j+=*s-'a'+10;
					s++;
				}
				BGBFR2_AsmParse_EmitChar(&t, j);
				continue;
			}

			if((s[0]=='\\') && (s[1]=='0'))
			{
				s++; j=0;
				while(*s && (*s>='0') && (*s<='7'))
					j=(j*8)+((*s++)-'0');
				BGBFR2_AsmParse_EmitChar(&t, j);
				continue;
			}

			if((s[0]=='\\') && (s[1]>='0') && (s[1]<='9'))
			{
				s++; j=0;
				while(*s && (*s>='0') && (*s<='0'))
					j=(j*10)+((*s++)-'0');
				BGBFR2_AsmParse_EmitChar(&t, j);
				continue;
			}

			if((*s=='\\') && BGBFR2_AsmParse_IsLineBreak(s+1))
			{
				s++;
				while(*s && (*s<=' '))
				{
					if((*s=='\r') || (*s=='\n'))
						break;
					s++;
				}
				if(*s=='\r')s++;
				if(*s=='\n')s++;
				continue;
			}

			if(*s=='\\')
			{
				s++;
				switch(*s++)
				{
				case 'a':
					*t++='\a';
					break;
				case 'b':
					*t++='\b';
					break;
				case 'e':
					*t++='\x1B';
					break;
				case 'f':
					*t++='\f';
					break;
				case 'n':
					*t++='\n';
					break;
				case 't':
					*t++='\t';
					break;
				case 'r':
					*t++='\r';
					break;
				case 'v':
					*t++='\v';
					break;
				case '\\':
					*t++='\\';
					break;
				case '"':
					*t++='"';
					break;
				case '\'':
					*t++='\'';
					break;
				case '?':
					*t++='?';
					break;
				default:
					break;
				}
				continue;
			}
//			*t++=*s++;
			j=BGBFR2_AsmParse_ParseChar(&s);
			BGBFR2_AsmParse_EmitChar(&t, j);
		}
		
		if(t>=te)
		{
			*t++=0;
			*ty=BGBFR2_TOKEN_STRING_OVF;
			return(s);
		}
		
		*t++=0;
		if(*s)s++;

		*ty=BGBFR2_TOKEN_STRING;
		if(i)*ty=BGBFR2_TOKEN_CHARSTRING;
		return(s);
	}

	if(!strncmp(s, "...", 3) ||
		!strncmp(s, "\"\"\"", 3) ||
		!strncmp(s, "\'\'\'", 3))
	{
		*t++=*s++;
		*t++=*s++;
		*t++=*s++;
		*t++=0;

		*ty=BGBFR2_TOKEN_MISC;
		return(s);
	}

	if((*s==',') || (*s==';') || ((*s=='.') && (s[1]!='.')))
	{
		*t++=*s++;
		*t++=0;

//		kprint("token: %p '%s'\n", s, b);
		*ty=BGBFR2_TOKEN_SEPERATOR;
		return(s);
	}

#if 0
	if(!strncmp(s, "<[[", 3) || !strncmp(s, "]]>", 3))
	{
		*t++=*s++;
		*t++=*s++;
		*t++=*s++;
		*t++=0;

//		kprint("token: %p '%s'\n", s, b);
		*ty=BGBFR2_TOKEN_BRACE;
		return(s);
	}
#endif

	if(
		!strncmp(s, "{:", 2) || !strncmp(s, ":}", 2) ||
		!strncmp(s, "[:", 2) || !strncmp(s, ":]", 2) ||
		!strncmp(s, "(:", 2) || !strncmp(s, ":)", 2))
	{
		*t++=*s++;
		*t++=*s++;
		*t++=0;

//		kprint("token: %p '%s'\n", s, b);
		*ty=BGBFR2_TOKEN_BRACE;
		return(s);
	}

	if(
		(*s=='{') || (*s=='}') ||
		(*s=='[') || (*s==']') ||
		(*s=='(') || (*s==')'))
	{
		*t++=*s++;
		*t++=0;

//		kprint("token: %p '%s'\n", s, b);
		*ty=BGBFR2_TOKEN_BRACE;
		return(s);
	}

	if(BGBFR2_AsmParse_OpChar(*s))
	{
		if(
			!strncmp(s, "<<<=", 4) ||
			!strncmp(s, ">>>=", 4) ||
			!strncmp(s, "::==", 4) ||
			!strncmp(s, "<<==", 4) ||
			!strncmp(s, ">>==", 4) ||
			!strncmp(s, ":<<=", 4) ||
			!strncmp(s, ":>>=", 4)
			)
		{
			*t++=*s++;
			*t++=*s++;
			*t++=*s++;
			*t++=*s++;
			*t++=0;
		}else if(
			!strncmp(s, "<<<", 3) ||
			!strncmp(s, ">>>", 3) ||
			!strncmp(s, "===", 3) ||
			!strncmp(s, "!==", 3) ||
			!strncmp(s, "<=>", 3) ||
			!strncmp(s, "<<=", 3) ||
			!strncmp(s, ">>=", 3) ||
			!strncmp(s, ":!=", 3) ||
			!strncmp(s, ":<=", 3) ||
			!strncmp(s, ":>=", 3) ||
			!strncmp(s, ":<<", 3) ||
			!strncmp(s, ":>>", 3)
			)
		{
			*t++=*s++;
			*t++=*s++;
			*t++=*s++;
			*t++=0;
		}else if(
			!strncmp(s, "==", 2) ||
			!strncmp(s, "<=", 2) ||
			!strncmp(s, ">=", 2) ||
			!strncmp(s, "!=", 2) ||
			!strncmp(s, "<<", 2) ||
			!strncmp(s, ">>", 2) ||
			!strncmp(s, "&&", 2) ||
			!strncmp(s, "||", 2) ||

			!strncmp(s, "..", 2) ||
			!strncmp(s, "**", 2) ||
			!strncmp(s, "%%", 2) ||
			!strncmp(s, "::", 2) ||

			!strncmp(s, "++", 2) ||
			!strncmp(s, "--", 2) ||

			!strncmp(s, "+=", 2) ||
			!strncmp(s, "-=", 2) ||
			!strncmp(s, "*=", 2) ||
			!strncmp(s, "/=", 2) ||
			!strncmp(s, "\\=", 2) ||
			!strncmp(s, "&=", 2) ||
			!strncmp(s, "|=", 2) ||
			!strncmp(s, "^=", 2) ||

			!strncmp(s, "+.", 2) ||
			!strncmp(s, "-.", 2) ||
			!strncmp(s, "*.", 2) ||
			!strncmp(s, "/.", 2) ||
			!strncmp(s, "\\.", 2) ||
			!strncmp(s, "&.", 2) ||
			!strncmp(s, "|.", 2) ||
			!strncmp(s, "^.", 2) ||

			!strncmp(s, "->", 2) ||

			!strncmp(s, ":=", 2) ||
			!strncmp(s, ":<", 2) ||
			!strncmp(s, ":>", 2)
			)
		{
			*t++=*s++;
			*t++=*s++;
			*t++=0;
		}else
		{
			*t++=*s++;
			*t++=0;
		}

//		kprint("token: %p '%s'\n", s, b);
		*ty=BGBFR2_TOKEN_OPERATOR;
		if(!strcmp(b, ":"))*ty=BGBFR2_TOKEN_SEPERATOR;
		if(!strcmp(b, "::"))*ty=BGBFR2_TOKEN_SEPERATOR;

		return(s);
	}

	BSVM_MSG_Error(s, "BGBFR2_AsmParse_Token: Parse Error Char '%c'\n", *s);
	*ty=BGBFR2_TOKEN_NULL;
	return(NULL);
}

char *BGBFR2_AsmParse_TokenQName(char *s, char *b, int *rty)
{
	char tb[256], tb2[256];
	char *t, *s1;
	int ty, ty2;

	s=BGBFR2_AsmParse_Token(s, tb, &ty);
	if(ty!=BGBFR2_TOKEN_NAME)
	{
		if(b)strcpy(b, tb);
		if(rty)*rty=ty;
		return(s);
	}

	if(!b)b=gcralloc(256);

	t=b;
	strcpy(t, tb);
	t+=strlen(t);
	
	s1=BGBFR2_AsmParse_Token(s, tb2, &ty2);
	s1=BGBFR2_AsmParse_Token(s1, tb, &ty);
	while(
		((!strcmp(tb2, "::") && (ty2==BGBFR2_TOKEN_SEPERATOR)) ||
		(!strcmp(tb2, ".") && (ty2==BGBFR2_TOKEN_SEPERATOR))) &&
		(ty==BGBFR2_TOKEN_NAME))
	{
		*t++='/';
		strcpy(t, tb);
		t+=strlen(t);

		s=s1;
		s1=BGBFR2_AsmParse_Token(s, tb2, &ty2);
		s1=BGBFR2_AsmParse_Token(s1, tb, &ty);
	}

	ty=BGBFR2_TOKEN_NAME;
	if(rty)*rty=ty;
	return(s);
}

char *BGBFR2_AsmParse_EatToken(char *s, char *b, int ty)
{
	char tb[256];
	char *s1;
	int ty2;

	s1=BGBFR2_AsmParse_Token(s, tb, &ty2);
	if(!s1)return(s1);

	if(ty && (ty!=ty2))
		return(s);
	if(b && strcmp(tb, b))
		return(s);
	return(s1);
}
