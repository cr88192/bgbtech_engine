...
#if 0
int FN(load_strings2)(char *name)
{
	char tb[4096];
	FILE *fd;
	char *s, *t;
...
u16 FN(CRC16_CCIT)(byte *data, int sz)
{
	byte *cs, *ce;
	u16 crc;
...
int FN(hash_string_0)(char *str)
	{ return(FN(CRC16_CCIT)(str, strlen(str))); }
int FN(hash_next_0)(int hi) { return(hi*127); }
int FN(hash_idx_0)(int hi)	{ return(hi); }


u16 FN(Hash16_BGB)(byte *data, int sz)
{
	byte *cs, *ce;
	u16 hash;
...
#if 1
int FN(hash_string_1)(char *str)
	{ return(FN(Hash16_BGB)(str, strlen(str))); }
int FN(hash_next_1)(int hi) { return(hi*127); }
int FN(hash_idx_1)(int hi)	{ return(hi); }
#endif

u16 FN(Hash16_BGB2)(byte *data, int sz)
{
	byte *cs, *ce;
	u16 hash;
...
#if 1
int FN(hash_string_2)(char *str)
	{ return(FN(Hash16_BGB2)(str, strlen(str))); }
int FN(hash_next_2)(int hi) { return(hi*127); }
int FN(hash_idx_2)(int hi)	{ return(hi); }
#endif
...
int FN(bscc_main)()
{
	...
	FN(load_strings)("filelst1.txt");
//	FN(load_strings2)("zpack.dll");
	...
	i=FN(CRC16_CCIT)(tst_strings[0], strlen(tst_strings[0]));
	printf("H0 %d\n", i);
