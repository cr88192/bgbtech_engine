...
#if 1
PDGL_API byte *PDJPG_Load(FILE *fd, int *xs, int *ys)
{
	byte *buf, *obuf;
	int fsz;
...
PDGL_API int PDJPG_Store(FILE *fd, byte *ibuf, int xs, int ys, int qf)
{
	static byte *buf=NULL;
	int fsz;
...
#if 1
byte *PDJPG_LoadRaw(char *name, int *w, int *h)
{
	FILE *fd;
	byte *tmp;
	...
	tmp=PDJPG_Load(fd, w, h);
	fclose(fd);

	return(tmp);
}

int PDJPG_SaveRaw(char *name, byte *buf, int xs, int ys)
{
	FILE *fd;
	int i;
	...
	PDJPG_Store(fd, buf, xs, ys, 90);
	fclose(fd);

	return(0);
}

int PDJPG_SaveRawFlip(char *name, byte *buf, int xs, int ys)
{
	FILE *fd;
	byte *buf1;
	int i, j, k, w, h, w2;
