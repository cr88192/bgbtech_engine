...
Basic Format 0:
    ...
    8 bit left-center max (truncated)
...
Basic Format 1:
	4 bit: Block-Mode (1)
	currently unused (4 bits, zeroed)
	8 bits (LCMin/LCMax, bits 4-7)
	16 bit min sample (center)
	16 bit max sample (center)
	8 bit left-center min (truncated)
	8 bit left-center max (truncated)
...
Stereo Format 5:
	...
	currently unused (4 bits, zeroed)
	8 bits (LCMin/LCMax, bits 4-7)
	16 bit min sample (side)
	16 bit max sample (side)
	8 bit center min (truncated)
	8 bit center max (truncated)
	16x 4-bit center (64 bits)
	32x 4-bit side (128 bits)

This directly codes samples, with the 4-bit values encoding even side-channel samples.
The 4-bit center value encodes the center channel.
...
#if 0
void BGBMID_BTAC_Encode64BlockHeaderS16(
	...
	int *rcm, int *rcn, int flags)
{
	...
	if(flags&BGBMID_BTAC_EFL_MIDSTREAM)
	{
		ba=0;
		for(i=-64; i<128; i++)
		{
...
//	p0=p2; p1=p2;
	...
	if(flags&(BGBMID_BTAC_EFL_CENTERMONO|BGBMID_BTAC_EFL_HALFMONO))
		{ p0=0; p1=0; }

//	p0/=2;	p1/=2;

	p0=bgbmid_spowi(p0, 0.93);
	p1=bgbmid_spowi(p1, 0.93);

//	p0=bgbmid_spowi(p0, 0.95);
//	p1=bgbmid_spowi(p1, 0.95);

//	p0=bgbmid_spowi(p0, 0.73);
//	p1=bgbmid_spowi(p1, 0.73);

//	p0=bgbmid_spowi(p0, 0.90);
//	p1=bgbmid_spowi(p1, 0.90);

//	p0=bgbmid_spowi(p0, 0.80);
//	p1=bgbmid_spowi(p1, 0.80);
...
void BGBMID_BTAC_Encode64BlockHeaderS16(
	...
	int *rcm, int *rcn, int flags)
{
	BGBMID_BTAC_Encode64BlockHeaderPanS16(
		iblk, oblk, tcblk, NULL,
		rcm, rcn, NULL, NULL, NULL, NULL, flags);
}

void BGBMID_BTAC_Encode64BlockHeaderPanS16(
	s16 *iblk, byte *oblk,
	s16 *tcblk, s16 *tcpan,
	int *rcm, int *rcn,
	int *rlm, int *rln,
...
//	p2=cm2-rm2;	p3=cn2-rn2;
	...
	for(i=0; i<4; i++)
	{
		p2=bgbmid_ssqri(bgbmid_ssqrti(p0));
		p3=bgbmid_ssqri(bgbmid_ssqrti(p1));
...
//	p0=p2; p1=p2;
	...
	if((flags&BGBMID_BTAC_EFL_CENTERMONO) || (flags&BGBMID_BTAC_EFL_HALFMONO))
		{ p0=0; p1=0; }
...
void BGBMID_BTAC_Encode64BlockHeaderSideS16(
	s16 *iblk, byte *oblk,
	s16 *tcblk, s16 *tcpan,
	int *rcm, int *rcn,
	int *rsm, int *rsn,
...
//	sm=bgbmid_ssqr(p0*1.4254)+0.5;
//	sn=bgbmid_ssqr(p1*1.4254)+0.5;
...
void BGBMID_BTAC_Decode64BlockHeader(byte *iblk, int flags,
...
//	i=bgbmid_ssqri((s8)iblk[6]);
//	j=bgbmid_ssqri((s8)iblk[7]);
...
void BGBMID_BTAC_Decode64BlockSideHeader(byte *iblk, int flags,
	int *rcm, int *rcn, int *rca,
...
//	sm=bgbmid_ssqr(((s8)iblk[6])*1.4254)+0.5;
//	sn=bgbmid_ssqr(((s8)iblk[7])*1.4254)+0.5;
...
//	int lm, ln, cm, rm, rn, cn, tl, tr, tc;
	...
	BGBMID_BTAC_Encode64BlockHeaderS16(iblk, oblk, tcblk, &cm, &cn, flags);
...
void BGBMID_BTAC_Decode64Block8S16(byte *iblk, s16 *oblk, int flags)
{
	...
	BGBMID_BTAC_Decode64BlockHeader(iblk, flags,
	...
	for(i=0; i<16; i++)
	{
		...
		if(flags&(BGBMID_BTAC_EFL_STEREO|BGBMID_BTAC_EFL_HALFMONO))
		{
			s0=(ln-lm);
			s1=(rn-rm);
			lm2=((s0*p0+8)>>4)+lm;
...
BMID_API void BGBMID_BTAC_DeblockStreamS16(
	s16 *oblk, int len)
{
	int p0, p1, p2, p3;
	int q0, q1, q2, q3;
...
void BGBMID_BTAC_Encode64BlockTagS16(s16 *iblk, byte *oblk,
	int tag, int flags)
{
	switch(tag)
	{
	case 0:
		BGBMID_BTAC_Encode64Block8S16(iblk, oblk, flags);
		break;
	case 1:
		BGBMID_BTAC_Encode64Block9S16(iblk, oblk, flags);
		break;
	case 2:
		BGBMID_BTAC_Encode64Block10S16(iblk, oblk, flags);
		break;
	case 3:
//		BGBMID_BTAC_Encode64Block12S16(iblk, oblk, flags);
		BGBMID_BTAC_Encode64Block14S16(iblk, oblk, flags);
		break;
	case 4:
		BGBMID_BTAC_Encode64Block13S16(iblk, oblk, flags);
		break;
	case 5:
		BGBMID_BTAC_Encode64Block15S16(iblk, oblk, flags);
		break;

//	case 5:
//		BGBMID_BTAC_Encode64Block11S16(iblk, oblk, flags);
//		break;
//	case 5:
//		BGBMID_BTAC_Encode64Block14S16(iblk, oblk, flags);
//		break;
...
void BGBMID_BTAC_Decode64BlockTagS16(byte *iblk, s16 *oblk, int flags)
{
	...
	{
	case 0:
		BGBMID_BTAC_Decode64Block8S16(iblk, oblk, flags);
		break;
	case 1:
		BGBMID_BTAC_Decode64Block9S16(iblk, oblk, flags);
		break;
	case 2:
		BGBMID_BTAC_Decode64Block10S16(iblk, oblk, flags);
		break;
	case 3:
//		BGBMID_BTAC_Decode64Block12S16(iblk, oblk, flags);
		BGBMID_BTAC_Decode64Block14S16(iblk, oblk, flags);
		break;
	case 4:
		BGBMID_BTAC_Decode64Block13S16(iblk, oblk, flags);
		break;
	case 5:
		BGBMID_BTAC_Decode64Block15S16(iblk, oblk, flags);
		break;

//	case 5:
//		BGBMID_BTAC_Decode64Block11S16(iblk, oblk, flags);
//		break;
//	case 5:
//		BGBMID_BTAC_Decode64Block14S16(iblk, oblk, flags);
//		break;
...
void BGBMID_BTAC_CompandBlockS16(s16 *iblk, s16 *oblk, int flags)
{
	...
	for(i=0; i<64; i++)
		for(j=0; j<2; j++)
	{
		oblk[i*2+j]=(int)(181.02*bgbmid_ssqrt(iblk[i*2+j]));
//		oblk[i*2+j]=(int)(128.0*bgbmid_ssqrt(iblk[i*2+j]));
//		oblk[i*2+j]=(int)(bgbmid_ssqrt(iblk[i*2+j]));
	}
}
...
void BGBMID_BTAC_UnCompandBlockS16(s16 *iblk, s16 *oblk, int flags)
{
	...
	for(i=0; i<64; i++)
		for(j=0; j<2; j++)
	{
		oblk[i*2+j]=(int)(bgbmid_ssqr(iblk[i*2+j]/181.02));
//		oblk[i*2+j]=(int)(bgbmid_ssqr(iblk[i*2+j]/128.0));
//		oblk[i*2+j]=(int)(bgbmid_ssqr(iblk[i*2+j]));
	}
}
#endif

BMID_API int BGBMID_BTAC_Calc64BlockErrorS16(
...
BMID_API int BGBMID_BTAC_Check64BlockBestS16(s16 *iblk, int flags)
{
	...
	for(i=0; i<6; i++)
	{
		BGBMID_BTAC_Encode64BlockTagS16(iblk, tblk, i, flags);
		BGBMID_BTAC_Decode64BlockTagS16(tblk, eblk, flags);
		te=BGBMID_BTAC_Calc64BlockErrorS16(iblk, eblk, flags);
		if(te<be) { bn=i; be=te; }
	}
...
void BGBMID_BTAC_Encode64BlockBestTagS16(s16 *iblk, byte *oblk, int flags)
{
	...
	if((flags&BGBMID_BTAC_EFL_STEREO) && (flags&BGBMID_BTAC_EFL_HALFMONO) &&
		((flags&BGBMID_BTAC_EBMODE_MASK)==BGBMID_BTAC_EBMODE_BLK64))
	{
		ltag=BGBMID_BTAC_Check64BlockBestS16(iblk+0, flags);
		rtag=BGBMID_BTAC_Check64BlockBestS16(iblk+1, flags);

		BGBMID_BTAC_Encode64BlockTagS16(
			iblk+0, oblk+ 0, ltag, flags);
		BGBMID_BTAC_Encode64BlockTagS16(
			iblk+1, oblk+32, rtag, flags);
	}else if(flags&BGBMID_BTAC_EFL_STEREO)
	{
		tag=BGBMID_BTAC_Check64BlockBestS16(iblk, flags);
		BGBMID_BTAC_Encode64BlockTagS16(iblk, oblk, tag, flags);
	}else
	{
		tag=BGBMID_BTAC_Check64BlockBestS16(iblk, flags);
		BGBMID_BTAC_Encode64BlockTagS16(iblk, oblk, tag, flags);
...
BMID_API void BGBMID_BTAC_Encode64StreamS16(
...
//			tag=0;
//			tag=2;	//2, 3
//			tag=BGBMID_BTAC_Check64BlockBestS16(
//				iblk+i*2*64+0, flags&(~BGBMID_BTAC_EFL_HALFMONO));

			ltag=BGBMID_BTAC_Check64BlockBestS16(iblk+i*2*64+0, flags);
			rtag=BGBMID_BTAC_Check64BlockBestS16(iblk+i*2*64+1, flags);
...
//			tag=5;
			...
			BGBMID_BTAC_Encode64BlockTagS16(iblk+i*2*64, oblk+i*32, tag, fl2);

//			BGBMID_BTAC_CompandBlockS16(iblk+i*2*64, tblk, flags);
//			tag=BGBMID_BTAC_Check64BlockBestS16(tblk, flags);
//			BGBMID_BTAC_Encode64BlockTagS16(tblk, oblk+i*32, tag, fl2);
...
//			printf("%d", tag);
		...
		}else
		{
			...
			BGBMID_BTAC_Encode64BlockTagS16(iblk+i*64, oblk+i*32, tag, fl2);
			tcnt[tag]++;
		}
	}
...
BMID_API void BGBMID_BTAC_Decode64StreamS16(
	byte *iblk, s16 *oblk, int len, int flags)
{
	...
	for(i=0; i<n; i++)
	{
		...
		{
//			BGBMID_BTAC_Decode64BlockTagS16(iblk+i*32, tblk, flags);
//			BGBMID_BTAC_UnCompandBlockS16(tblk, oblk+i*2*64, flags);

			BGBMID_BTAC_Decode64BlockTagS16(iblk+i*32, oblk+i*2*64, flags);
		}else
		{
			BGBMID_BTAC_Decode64BlockTagS16(iblk+i*32, oblk+i*64, flags);
		}
	}
	...
	if((flags&BGBMID_BTAC_EFL_STEREO) && (flags&BGBMID_BTAC_EFL_HALFMONO))
	{
		BGBMID_BTAC_DeblockStreamS16(oblk, len);
		BGBMID_BTAC_FilterStreamS16(oblk, len);
	}else if(flags&BGBMID_BTAC_EFL_STEREO)
	{
		BGBMID_BTAC_DeblockStreamS16(oblk, len);
		BGBMID_BTAC_FilterStreamS16(oblk, len);
	...
	{
		BGBMID_BTAC_DeblockStreamMonoS16(oblk, len);
		BGBMID_BTAC_FilterStreamMonoS16(oblk, len);
	}
#endif
}
...
void BGBMID_BTAC_DecodeBlockTagS16(byte *iblk, s16 *oblk, int flags)
{
	...
	{
		BGBMID_BTAC_Decode64BlockTagS16(iblk, oblk, flags);
		return;
	}

	if(((flags&BGBMID_BTAC_EBMODE_MASK)==BGBMID_BTAC_EBMODE_BLK256) ||
		((flags&BGBMID_BTAC_EBMODE_MASK)==BGBMID_BTAC_EBMODE_BLK256LQ) ||
		((flags&BGBMID_BTAC_EBMODE_MASK)==BGBMID_BTAC_EBMODE_BLK256HQ))
	{
		BGBMID_BTAC_Decode256BlockTagS16(iblk, oblk, flags);
		return;
	}
}
...
//	if(1)
	{
		BGBMID_BTAC_DecodeBlockTagS16(iblk, oblk, flags);
		return;
	}
		...
		((flags&BGBMID_BTAC_EBMODE_MASK)==BGBMID_BTAC_EBMODE_BLK256HQ))
	{
		...
		{
			BGBMID_BTAC_Decode256BlockTagS16(iblk-bs, tblk+0*256*2, flags);
			BGBMID_BTAC_Decode256BlockTagS16(iblk, tblk+1*256*2, flags);
			BGBMID_BTAC_Decode256BlockTagS16(iblk+bs, tblk+2*256*2, flags);

			BGBMID_BTAC_DeblockStreamS16(tblk, 3*256);
			BGBMID_BTAC_FilterStreamS16(tblk, 3*256);
		
			for(i=0; i<256; i++)
			{
				oblk[i*2+0]=tblk[1*256*2+i*2+0];
			}
		}else if(flags&BGBMID_BTAC_EFL_STEREO)
		{
			BGBMID_BTAC_Decode256BlockTagS16(iblk-bs, tblk+0*256*2, flags);
			...
			{
				oblk[i*2+0]=tblk[1*256*2+i*2+0];
				oblk[i*2+1]=tblk[1*256*2+i*2+1];
			}
		}else
		{
			BGBMID_BTAC_Decode256BlockTagS16(iblk-bs, tblk+0*256, flags);
			BGBMID_BTAC_Decode256BlockTagS16(iblk, tblk+1*256, flags);
			BGBMID_BTAC_Decode256BlockTagS16(iblk+bs, tblk+2*256, flags);

			BGBMID_BTAC_DeblockStreamMonoS16(tblk, 3*256);
			BGBMID_BTAC_FilterStreamMonoS16(tblk, 3*256);
...
//		return;
	...
	if((flags&BGBMID_BTAC_EFL_STEREO) && (flags&BGBMID_BTAC_EFL_HALFMONO))
	{
		BGBMID_BTAC_Decode64BlockTagS16(iblk-32, tblk+0*64*2, flags);
		BGBMID_BTAC_Decode64BlockTagS16(iblk, tblk+1*64*2, flags);
		BGBMID_BTAC_Decode64BlockTagS16(iblk+32, tblk+2*64*2, flags);

		BGBMID_BTAC_DeblockStreamS16(tblk, 3*64);
		BGBMID_BTAC_FilterStreamS16(tblk, 3*64);
		
		for(i=0; i<64; i++)
		{
			oblk[i*2+0]=tblk[1*64*2+i*2+0];
		}
	}else if(flags&BGBMID_BTAC_EFL_STEREO)
	{
		BGBMID_BTAC_Decode64BlockTagS16(iblk-32, tblk+0*64*2, flags);
		...
		{
			oblk[i*2+0]=tblk[1*64*2+i*2+0];
			oblk[i*2+1]=tblk[1*64*2+i*2+1];
		}
	}else
	{
		BGBMID_BTAC_Decode64BlockTagS16(iblk-32, tblk+0*64, flags);
		BGBMID_BTAC_Decode64BlockTagS16(iblk, tblk+1*64, flags);
		BGBMID_BTAC_Decode64BlockTagS16(iblk+32, tblk+2*64, flags);

		BGBMID_BTAC_DeblockStreamMonoS16(tblk, 3*64);
		BGBMID_BTAC_FilterStreamMonoS16(tblk, 3*64);
...
BMID_API void BGBMID_BTAC_EncodeStreamS16(
	s16 *iblk, byte *oblk, int len, int flags)
{
	if((flags&BGBMID_BTAC_EBMODE_MASK)==BGBMID_BTAC_EBMODE_BLK64)
	{
		BGBMID_BTAC_Encode64StreamS16(iblk, oblk, len, flags);
		return;
	}
	...
	if(((flags&BGBMID_BTAC_EBMODE_MASK)==BGBMID_BTAC_EBMODE_BLK256) ||
		...
		((flags&BGBMID_BTAC_EBMODE_MASK)==BGBMID_BTAC_EBMODE_BLK256HQ))
	{
		BGBMID_BTAC_Encode256StreamS16(iblk, oblk, len, flags);
		return;
	}
}

BMID_API void BGBMID_BTAC_DecodeStreamS16(
	byte *iblk, s16 *oblk, int len, int flags)
{
	if((flags&BGBMID_BTAC_EBMODE_MASK)==BGBMID_BTAC_EBMODE_BLK64)
	{
		BGBMID_BTAC_Decode64StreamS16(iblk, oblk, len, flags);
		return;
	}
	...
	if(((flags&BGBMID_BTAC_EBMODE_MASK)==BGBMID_BTAC_EBMODE_BLK256) ||
		...
		((flags&BGBMID_BTAC_EBMODE_MASK)==BGBMID_BTAC_EBMODE_BLK256HQ))
	{
		BGBMID_BTAC_Decode256StreamS16(iblk, oblk, len, flags);
		return;
	}
}
