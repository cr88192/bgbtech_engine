...
//				csmp->pcm[j]=(buf[j*2+0]<<8)|buf[j*2+1];
		...
		{
			csmp->pcm=malloc(tsmp[i].size * sizeof(short));
...
int MID_TestPatchFilt(char *name, Sample *samp)
{
	...
	{
		j=cur->len*(1.0/M_E);
		buf0=malloc(j*sizeof(short));
		buf1=malloc(cur->len*sizeof(short));
...
//	mid_ticksamp=11025.0/k;
	...
	mid_pcmbuf=malloc(1<<27);
	mid_pcmbufe=mid_pcmbuf;

	mid_pcmbuft=malloc(1<<20);
