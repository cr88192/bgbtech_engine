/*
Redesigned to be more friendly with non-SSE version.
New goal is to be function call compatible, even if potentially less efficient.
 */
...
#define v2_to_m128(v)	_mm_loadu_ps((float *)(&(v)))
#define v3_to_m128(v)	_mm_loadu_ps((float *)(&(v)))
#define v4_to_m128(v)	_mm_loadu_ps((float *)(&(v)))
#define q_to_m128(v)	_mm_loadu_ps((float *)(&(v)))

#define vec2_m128(v)	_vec2_m128(v)
#define vec3_m128(v)	_vec3_m128(v)
#define vec4_m128(v)	_vec4_m128(v)
#define quat_m128(v)	_vec4_m128(v)
...
#if 1
static vec2u _vec2_m128(__m128 m)
{
	vec2u t;
	_mm_storeu_ps((float *)(&t), m);
	return(t);
//	return(_vec2i(x[0], x[1]));
};

static vec3u _vec3_m128(__m128 m)
{
	vec3u t;
	_mm_storeu_ps((float *)(&t), m);
	return(t);
...
static vec4u _vec4_m128(__m128 m)
{
	vec4u t;
	_mm_storeu_ps((float *)(&t), m);
	return(t);
...
//	a=_mm_loadu_ps((float *)(&v));
//	t=_mm_shuffle_ps(a, a, sh);
//	_mm_storeu_ps((float *)(&b), t);
...
{
	vec3u c;
	_mm_storeu_ps((float *)(&c),
		_mm_mul_ps(
			_mm_loadu_ps((float *)(&a)),
			_mm_loadu_ps((float *)(&b))));
	return(c.x+c.y+c.z);
...
static float v2len2(vec2u a)
	{ return(v2dot(a, a)); }
static float v3len2(vec3u a)
	{ return(v3dot(a, a)); }
static float v4len2(vec4u a)
	{ return(v4dot(a, a)); }
static float qlen2(quatu a)
	{ return(qdot(a, a)); }
...
static vec4 v4sub(vec4u a, vec4u b)
{
	...
	_mm_storeu_ps((float *)(&c),
		_mm_sub_ps(
			_mm_loadu_ps((float *)(&a)),
			_mm_loadu_ps((float *)(&b))));
//	c;c.x=a.x-b.x;c.y=a.y-b.y;c.z=a.z-b.z;c.w=a.w-b.w;
	return(c);
}
