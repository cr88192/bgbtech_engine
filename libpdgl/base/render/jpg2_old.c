...
#if 0
ushort ctx->huff_code[8*256];
byte ctx->huff_len[8*256];

byte *ctx->huff_cs;	//current pos in bitstream (input)
byte *ctx->huff_ct;	//current pos in bitstream (output)
uint ctx->huff_win;	//bitstream window
int ctx->huff_pos;	//bitstream offset
int ctx->huff_isend;	//bitstream has broken (decoder)

byte ctx->jpg_qt[4][64];	//quantization tables
int ctx->jpg_qtfp[4][64];	//quantization tables (fixed point)
...
int ctx->jpg_scid[4];
int ctx->jpg_scn[4];
short *ctx->jpg_scbuf[4];
byte *ctx->jpg_sibuf[4];

int ctx->jpg_schs[4];	//scan horizontal size
int ctx->jpg_scvs[4];	//scan vertical size
float ctx->jpg_schsc[4];	//scan horizontal scale
float ctx->jpg_scvsc[4];	//scan vertical scale

//Encoder Specific
int ctx->jpg_mono;
int ctx->jpg_tabcacheframe;	//cache table frame count.
#endif
...
void PDJHUFF_WriteNBits(PDJPG_Context *ctx, int i, int n)
{
	i&=(1<<n)-1;
...
PDGL_API int PDJPG_EncodeCtx(
	PDJPG_Context *ctx, byte *ibuf, byte *obuf, int xs, int ys, int qf)
{
...
//	printf("M1\n");
	...
	j=(xs2/8)*(ys2/8);
	PDJPG_MakeQuantTabInput(ydb, j, ctx->jpg_qt[0], qf/100.0);

	j=(xs3/8)*(ys3/8);
	PDJPG_MakeQuantTabInput(udb, j, ctx->jpg_qt[1], qf/100.0);
	PDJPG_MakeQuantTabInput(vdb, j, ctx->jpg_qt[2], qf/100.0);
...
//	free(vb);
	...
	if(ctx->jpg_mono)
	{
		...
		for(i=0; i<j; i++)
		{
			PDJHUFF_QuantBlock(ctx, ydb+i*64, ydb+i*64, 0);
	...
	for(i=0; i<j; i++)
	{
		PDJHUFF_QuantBlock(ctx, udb+i*64, udb+i*64, 1);
		PDJHUFF_QuantBlock(ctx, vdb+i*64, vdb+i*64, 1);
...
//	printf("M2\n");
	...
	*ctx->huff_ct++=JPG_SOI;
	...
	{
		j=(xs2/8)*(ys2/8);
		for(i=0; i<j; i++)PDJHUFF_EncodeBlock(ctx, ydb+i*64, 0, 1);
	}else
	{
		for(i=0; i<=((ys3)/8); i++)
			for(j=0; j<(xs3/8); j++)
		{
			PDJHUFF_EncodeBlock(ctx, ydb+((i*2+0)*(xs2/8)+j*2+0)*64, 0, 1);
			PDJHUFF_EncodeBlock(ctx, ydb+((i*2+0)*(xs2/8)+j*2+1)*64, 0, 1);
			PDJHUFF_EncodeBlock(ctx, ydb+((i*2+1)*(xs2/8)+j*2+0)*64, 0, 1);
			PDJHUFF_EncodeBlock(ctx, ydb+((i*2+1)*(xs2/8)+j*2+1)*64, 0, 1);

			k=i*(xs3/8)+j;
			PDJHUFF_EncodeBlock(ctx, udb+k*64, 2, 3);
			PDJHUFF_EncodeBlock(ctx, vdb+k*64, 2, 3);
		}
	}
	...
	byte *ibuf, byte *obuf, int xs, int ys, int qf)
{
	...
	ctx=PDJPG_AllocContext();
	sz=PDJPG_EncodeCtx(ctx, ibuf, obuf, xs, ys, qf);
	PDJPG_FreeContext(ctx);
	return(sz);
}
