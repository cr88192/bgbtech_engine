...
#define PDGL_MAX_TEXTURES 16384
unsigned char texturebm[PDGL_MAX_TEXTURES/8];
int lasttexture;
short tex_width[PDGL_MAX_TEXTURES], tex_height[PDGL_MAX_TEXTURES];
byte *tex_buffer[PDGL_MAX_TEXTURES];
char *tex_name[PDGL_MAX_TEXTURES];
int tex_chain[PDGL_MAX_TEXTURES];
...
//	memset(texturebm, 0, PDGL_MAX_TEXTURES/8);
	lasttexture=1;
	return(0);
}
...
PDGL_API int Tex_LoadTextureFragment(int xs, int ys, byte *buf,
...
//faster but uglier
PDGL_API int Tex_ResampleQuick
...
PDGL_API int Tex_ResampleMono(byte *src, int iw, int ih,
...
PDGL_API int Tex_PadResample(int *src, int iw, int ih,
	int *dst, int ow, int oh)
{
	int i, j;
...
PDGL_API int Tex_SincPolateRGBA8(byte *src, int w, int h,
...
PDGL_API int Tex_ResampleSinc(byte *src, int iw, int ih,
	byte *dst, int ow, int oh)
{
	float tv[4];
	float xs, ys, xc, yc;
...
PDGL_API int Tex_ResampleSpline(byte *src, int iw, int ih,
	byte *dst, int ow, int oh)
{
	float tv[4];
	float xs, ys, xc, yc;
...
PDGL_API int Tex_FilterHBlur(byte *src, byte *dst,
...
PDGL_API int Tex_FilterBlurHV(byte *src, byte *dst,
...
PDGL_API int Tex_ResampleSpline2(byte *src, int iw, int ih,
	byte *dst, int ow, int oh)
{
	byte *buf;
	float xf, yf;
...
PDGL_API int Tex_LoadTexture2(int w, int h, byte *buf,
...
PDGL_API int Tex_LoadTexture3A(int *wp, int *hp, byte *buf,
	int num, int clrs)
{ return(Tex_LoadTexture3B(wp, hp, buf, num, clrs, 0)); }

PDGL_API int Tex_LoadTexture3B(int *wp, int *hp, byte *buf,
...
#if 0
	rsbuf=malloc(sz);
	memcpy(rsbuf, buf, sz); tl=1;
	while((tw>1) && (th>1))
	{
		BGBBTJ_BCnMip_MergeDXT5(rsbuf, rsbuf, tw, th);
		tw>>=1; th>>=1;

		sz=(tw/4)*(th/4)*blksz;
		if(!sz)sz=blksz;
		pdglCompressedTexImage2D(GL_TEXTURE_2D, tl, 
			cmp, tw, th, 0, sz, rsbuf);
		tl++;
	}
#endif
...
PDGL_API byte *Img_FlattenDIB(byte *img, int w, int h)
{
	...
	if(!init)
	{
//		gctalloc("img_dib_t", "byte+");
		init=1;
	}

	buf=gctalloc("img_dib_t", (w*h*3)+sizeof(BMPInfoHeader));
...
//		gctalloc("img_dib_t", "byte+");
	...
	n=((w+3)/4)*((h+3)/4);
	buf=gctalloc("img_bmp_t", (n*2*16)+sizeof(BMPHeader));
	ihead=(BMPHeader *)buf;
...
PDGL_API byte *Img_TryLoadTextureBPX(
...
#if 1
PDGL_API int Img_SaveTextureCacheBPX(
...
PDGL_API byte *Tex_LoadFileExtAlphaRaw(
...
PDGL_API byte *Tex_LoadFileExtBaseSuffixRaw(
...
PDGL_API int Tex_LoadFileExtBaseSuffix(
	char *name, char *ext, char *base, char *suff, int *w, int *h)
{
	char tbn[256];
	byte *buf;
...
PDGL_API int Tex_LoadFileBaseSuffix(
...
PDGL_API byte *Tex_LoadImageBufferRaw(char *name, char *type,
...
//	GfxDrv_GetWindowTexSize(&txs, &tys);
	...
	PDGL_GetViewOrigin(&xo, &yo);
	PDGL_GetViewSize(&xs, &ys);
//	GfxDrv_GetWindowTexSize(&txs, &tys);
	GfxDrv_GetWindowMaxTexSize(&txs, &tys);
