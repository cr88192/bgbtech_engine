...
//	if(PDGLUI_KeyDown(K_SHIFT))printf("shift a1\n");
...
//			b=PDGLUI_ClipBoard_Import("string_t");
		...
		if(PDGLUI_KeyDown(K_CTRL) || PDGLUI_KeyDown(K_ALT))
		{
...
//	if(ch)PDGLUI_Forms_Message(obj->form, obj->id, "changed", 0, &p);
	...
	if(ch)PDGLUI_Widget_AddEvent(obj, "changed");
	return(0);
}
...
PDGLUI_WidgetType pdglui_textarea_methods=
{
NULL,
"textarea",

PDGLUI_TextArea_Render,
PDGLUI_TextArea_HandleInput,
NULL,
PDGLUI_TextArea_Begin,
PDGLUI_TextArea_End,
NULL,
PDGLUI_TextArea_GetPropertyS,
PDGLUI_TextArea_SetPropertyS,
PDGLUI_TextArea_GetPropertyI,
PDGLUI_TextArea_SetPropertyI
};

int PDGLUI_TextArea_Init()
{
	PDGLUI_Widget_RegisterType(&pdglui_textarea_methods);
	return(0);
}
