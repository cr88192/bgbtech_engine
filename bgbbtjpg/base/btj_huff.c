...
int BGBBTJ_Huff_QuantCheckRound(
	BGBBTJ_JPG_Context *ctx, int a, int b, int la)
{
	int i, j;
...
//		obuf[i]=((ibuf[i]*ctx->jpg_qtfp[qid][i])+2048)>>12;
	...
	for(i=0; i<64; i++)
	{
		j=((ibuf[i]*ctx->jpg_qtfp[qid][i])+2048)>>12;
		k=(ibuf[i]*ctx->jpg_qtfp[qid][i])>>12;

		if(i)
			{ j=BGBBTJ_Huff_QuantCheckRound(ctx, j, k, lj); }
...
void BGBBTJ_Huff_DequantBlock(
	BGBBTJ_JPG_Context *ctx, short *ibuf, short *obuf,
	int qid, int rtmode)
{
	short *ib, *ob;
	byte *qt, *qt1;
...
//		obuf[i]=ibuf[i]*ctx->jpg_qt[qid][i];
	...
	if(rtmode==2)
	{
		obuf[0]=ibuf[0]*ctx->jpg_qt[qid][0];
		return;
	}
...
void BGBBTJ_Huff_DequantBlock_Flat(
	BGBBTJ_JPG_Context *ctx, short *ibuf, short *obuf, int qid)
{
	int i, j;
	obuf[0]=ibuf[0]*ctx->jpg_qt[qid][0];
