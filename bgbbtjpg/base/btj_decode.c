	...
	if(ctx->jpg_nc==1)
	{
		...
		for(i=0; i<n; i++)
		{
			...
			BGBBTJ_Huff_DequantBlock(ctx, dbuf,
				ctx->jpg_scbuf[0]+i*64, qid[0], rt);
			BGBBTJ_JPG_TransIDCT(
				ctx->jpg_scbuf[0]+i*64,
				ctx->jpg_sibuf[0]+i*64, 128, rt);
		}
	}else
...
//	if(0)
	{
		...
		for(i=0; i<n; i++)
		{
			...
			if(rt<0)break;
			BGBBTJ_Huff_DequantBlock(ctx, dbuf,
				ctx->jpg_scbuf[0]+k*64, qid[0], rt);
			BGBBTJ_JPG_TransIDCT(
				ctx->jpg_scbuf[0]+k*64,
				ctx->jpg_sibuf[0]+k*64, 128, rt);
				...
				for(k=0; k<cv[j]; k++)
				{
					for(l=0; l<ch[j]; l++)
					{
						...
						BGBBTJ_Huff_DequantBlock(ctx, dbuf+j*64,
							ctx->jpg_scbuf[j]+k1*64, qid[j], rt);
						
						if(ctx->jpg_rdct)
						{
							BGBBTJ_JPG_TransIRDCT(ctx->jpg_scbuf[j]+k1*64,
								ctx->jpg_sibuf[j]+k1*64, 128, rt);
						}else
						{
