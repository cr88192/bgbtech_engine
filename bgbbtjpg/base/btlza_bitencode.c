...
int BTLZA_BitEnc_BuildLengthsAdjust(int *stat, int nc, byte *cl, int ml)
{
	...
	while(1)
	{
		...
		if(j<0)
			printf("BTLZA_BitEnc_BuildLengthsAdjust: Huff Fail %d\n");
		...
		printf("BTLZA_BitEnc_BuildLengthsAdjust: Fiddle Adjust\n");
		for(i=0; i<nc; i++)
			stat[i]++;
		continue;
	}
	...
	byte *cs, int *lstat, int *dstat)
{
	...
	if(i>=60)
	{
		printf("BTLZA_BitEnc_StatLZRun: No Length %d\n", l);
	}
	...
	if(i>=64)
	{
		printf("BTLZA_BitEnc_StatLZRun: No Dist %d\n", l);
	}
}
...
void BTLZA_BitEnc_EncodeSymbol(BGBBTJ_BTLZA_Context *ctx, int v)
{
	if(!ctx->bs_ltab_len[v])
		printf("BTLZA_BitEnc_EncodeSymbol: no symbol %d\n", v);
	...
	int isz, int osz, int lvl)
{
	...
	if(j!=isz)
	{
		printf("BTLZA_BitEnc_EncodeStream64LvlZlTest: "
			"Size %d -> %d\n", isz, j);
		free(tbuf);
		return(-1);
	}
