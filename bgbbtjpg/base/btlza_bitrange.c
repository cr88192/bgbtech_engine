...
#if 0
	if(ctx->lz_wbuf)gcfree(ctx->lz_wbuf);
	if(ctx->lz_lbuf)gcfree(ctx->lz_lbuf);
	if(ctx->lz_hash)gcfree(ctx->lz_hash);

	if(ctx->lz_tbuf)gcfree(ctx->lz_tbuf);
	if(ctx->lz_mbuf)gcfree(ctx->lz_mbuf);

	if(ctx->mdl_lit)gcfree(ctx->mdl_lit);
	if(ctx->mdl_dist)gcfree(ctx->mdl_dist);
	if(ctx->mdl_xbits)gcfree(ctx->mdl_xbits);
	gcfree(ctx);
#endif
}
	...
	BGBBTJ_BTLZA_Context *ctx, int bits)
{
	...
	j=1<<bits;
	if(!ctx->mdl_lit)
		{ ctx->mdl_lit=gcalloc(j*sizeof(btlza_prob)); }
	ctx->ctxbits=bits;
	...
	j=1<<12;
	if(!ctx->mdl_dist)
		{ ctx->mdl_dist=gcalloc(j*sizeof(btlza_prob)); }
	if(!ctx->mdl_xbits)
		{ ctx->mdl_xbits=gcalloc(j*sizeof(btlza_prob)); }
	...
	BGBBTJ_BTLZA_Context *ctx, int bits)
{
	...
	j=1<<bits;
	if(!ctx->mdl_rbits)
		{ ctx->mdl_rbits=gcalloc(j*sizeof(btlza_prob)); }
