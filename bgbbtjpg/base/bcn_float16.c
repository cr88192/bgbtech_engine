...
u32 bgbbtj_float16_mantissatable[2048];
u32 bgbbtj_float16_exponenttable[64];
short bgbbtj_float16_offsettable[64];

u16 bgbbtj_float16_basetable[512];
byte bgbbtj_float16_shifttable[512];
...
BGBBTJ_API int BGBBTJ_Float16_Init()
{
	...
	for(i=0; i<1024; i++)
		{ bgbbtj_float16_mantissatable[i]=BGBBTJ_Float16_ConvertMantissa(i); }
	for(i=1024; i<2048; i++)
		{ bgbbtj_float16_mantissatable[i]=0x38000000U+((i-1024)<<13); }
	for(i=0; i<31; i++)
		{ bgbbtj_float16_exponenttable[i]=0x00000000U+(i<<23); }
	for(i=32; i<63; i++)
		{ bgbbtj_float16_exponenttable[i]=0x80000000U+((i-32)<<23); }
...
//	bgbbtj_float16_exponenttable[63]=0xC7800000U;
	...
	for(i=0; i<256; i++)
	{
		...
		if(e<-24)	//zeroes
		{
			bgbbtj_float16_basetable[i|0x000]=0x0000;	//-0
			bgbbtj_float16_basetable[i|0x100]=0x8000;	//+0
			bgbbtj_float16_shifttable[i|0x000]=24;
			bgbbtj_float16_shifttable[i|0x100]=24;
		}
		else if(e<-14)	//denormals
		{
			bgbbtj_float16_basetable[i|0x000]=(0x0400>>(-e-14))|0x0000;
			bgbbtj_float16_basetable[i|0x100]=(0x0400>>(-e-14))|0x8000;
			bgbbtj_float16_shifttable[i|0x000]=-e-1;
			bgbbtj_float16_shifttable[i|0x100]=-e-1;
		}
		else if(e<=15) //normal range
		{
			bgbbtj_float16_basetable[i|0x000]=((e+15)<<10)|0x0000;
			bgbbtj_float16_basetable[i|0x100]=((e+15)<<10)|0x8000;
			bgbbtj_float16_shifttable[i|0x000]=13;
			bgbbtj_float16_shifttable[i|0x100]=13;
		}
		else if(e<128)	//overflow to Inf
		{
			bgbbtj_float16_basetable[i|0x000]=0x7C00;
			bgbbtj_float16_basetable[i|0x100]=0xFC00;
			bgbbtj_float16_shifttable[i|0x000]=24;
			bgbbtj_float16_shifttable[i|0x100]=24;
		}
		else	//Inf and NaN
...
BGBBTJ_API double BGBBTJ_DecodeFloat16(u16 val)
{
	u32 f, h;
	h=val;
	f=bgbbtj_float16_mantissatable[
		bgbbtj_float16_offsettable[h>>10]+(h&0x3FF)]+
		bgbbtj_float16_exponenttable[h>>10];
	return(*(float *)(&f));
}
...
BGBBTJ_API u16 BGBBTJ_EncodeFloat16(double val)
{
	...
	*(float *)(&f)=val;
	...
	t=bgbbtj_float16_basetable[i]+
		((f&0x007FFFFF)>>bgbbtj_float16_shifttable[i]);
	return(t);
}
...
{
	u32 f;
	f=bgbbtj_float16_exponenttable[val>>10]|((val&0x3FF)<<13);
	return(*(float *)(&f));
}

BGBBTJ_API double BGBBTJ_DecodeFloat12(u16 val)
{
	return(BGBBTJ_DecodeFloat16(val<<4));
}

BGBBTJ_API u16 BGBBTJ_EncodeFloat12(double val)
{
	return((BGBBTJ_EncodeFloat16(val)+7)>>4);
}
...
{
	return(BGBBTJ_AddFloat16(x, y^0x8000));
}
...
{
	return(BGBBTJ_MulFloat16(x, BGBBTJ_RcpFloat16(y)));
}
