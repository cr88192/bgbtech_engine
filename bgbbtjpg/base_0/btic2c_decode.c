	...
	s32 *img, int xo, int yo, int xs, int ys, int blkt)
{
	...
	for(i=0; i<8; i++)
	{
		...
		ct[2]=cs0[2]+cs1[0];	ct[3]=cs0[3]+cs1[3];
		ct[4]=cs0[4]+cs1[0];	ct[5]=cs0[5]+cs1[5];
		ct[6]=cs0[6]+cs1[0];	ct[7]=cs0[7]+cs1[7];
		ct+=8; cs0+=8; cs1+=8;
	}
}
...
void BTIC2C_TransIBLKS_Flat(s32 *iblk, s16 *oblk, int blkt)
{
	...
	BTIC2C_TransIBLKT_Flat(iblk, s, blkt);
	cs=s; ct=oblk;
	for(i=0; i<8; i++)
	{
		ct[0]=cs[0];	ct[1]=cs[1];
...
#if 0
	i=(pdjhuff_win>>(20-pdjhuff_pos))&4095;
	j=ctx->huff_idx12[(tab<<12)|i];
	k=ctx->huff_len[(tab<<8)|j];
	if(k<=12)
	{
		pdjhuff_pos+=k;
		while(pdjhuff_pos>=8)
...
int BTIC2CH_DecodeDiffDC(BTIC2C_Context *ctx, int tab, int cn)
{
	int arg[16];
	int cmd, narg;
	int i, j, k;
	...
	while(i>=64)
	{
		...
		if(j==4)
		{
			...
			{
				j=i&31;
				k=btic2c_dbase[j]+BTIC2CH_ReadNBits(ctx, btic2c_dextra[j]);
				k=(k&1)?(-((k+1)>>1)):(k>>1);
				arg[narg++]=k;
				i=BTIC2CH_DecodeSymbol(ctx, tab);
			}
			...
			case 2:
				...
				break;
			case 3:
				ctx->blk_move[0]=arg[0];
				ctx->blk_move[1]=arg[1];
				break;
			default:
				break;
			}
...
//	for(i=1; i<64; i++)buf[i]=0;
	...
	buf[0]+=BTIC2CH_DecodeDiffDC(ctx, dctab, cn);
	if(ctx->skip_cnt)
		return(1);
...
//									scb, sjb, ctx->img_blktrans);
							...
							}else if(ctx->blk_mode[j]==BTIC2C_BLKM_REPLACE)
							{
								BTIC2CH_DequantBlock_Flat(ctx, sdb,
									scb, qid[j]);
//								BTIC2C_TransIWHT_Flat(
//									ctx->img_scbuf[j]+k1*64,
//									ctx->img_sibuf[j]+k1*64);
								if(sjb)
								{
									BTIC2C_TransIBLKS_Flat(
										scb, sjb, ctx->img_blktrans);
...
//					BTIC2C_TransIDCT(ctx->img_scbuf[j]+k1*64,
//						ctx->img_sibuf[j]+k1*64);
...
//							ctx->img_sibuf[j]+k1*64);
					...
					}else if(ctx->blk_mode[j]==BTIC2C_BLKM_MOVEADD)
					{
						...
						}else
						{
							BTIC2C_TransIBLKT_MoveAdd(
								scb, sib,
								ctx->img_sibuf[j],
								(j1+l)*8+ctx->blk_move[0],
								(i1+k)*8+ctx->blk_move[1],
								xi[j], yi[j],
								ctx->img_blktrans);
						}
			...
			{
				j=i%bw; k=i/bw;
//				BTIC2C_DecodeColorTransformYCbCr420_Block(ctx,
//					obuf, j*16, k*16, clrs);
...
//	ts=BTIC2CH_ReadNBits(ctx, 4);
//	rk=BTIC2CH_ReadNBits(ctx, 4);
...
//				pdjpg_marker[j], buf[i+1]);
			...
			if(buf[i+1]==JPG_DQT)BTIC2C_MarkerDQT(ctx, buf+i+2);
			if(buf[i+1]==JPG_DHT)BTIC2C_MarkerDHT(ctx, buf+i+2);
			if(buf[i+1]==JPG_SOF0)BTIC2C_MarkerSOF0(ctx, buf+i+2);
			if(buf[i+1]==JPG_SOS)BTIC2C_MarkerSOS(ctx, buf+i+2);

			if(buf[i+1]==JPG_APP0)BTIC2C_MarkerAPP0(ctx, buf+i+2);
			if(buf[i+1]==JPG_APP11)BTIC2C_MarkerAPP11(ctx, buf+i+2);
...
//	BTIC2C_DecodeColorTransform(ctx, obuf, clrs);
...
#if 0
// byte *BTIC2C_Decode(byte *buf, int sz, int *rxs, int *rys)
{
	BTIC2C_Context *ctx;
	byte *obuf;
	byte *otbuf;
	...
	obuf=BTIC2C_DecodeCtx(ctx, buf, sz, rxs, rys);
	ctx->img_imgbuf=NULL;
