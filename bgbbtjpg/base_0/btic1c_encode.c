	...
	BGBBTJ_BTIC1C_ImageInfo *ctx, byte *ct, int mode)
{
	...
	for(i=0; i<8; i++)
	{
		if(ctx->modes[i]==mode)
		{
			if(ctx->modecnt[mode]>ctx->modecnt[ctx->curmode])
//			if(0)
			{
				//switch mode
				*ct++=0xF0;
...
//		if(0)
		{
			...
			pa0=BGBBTJ_BT1C_EncodePredictDiffColorA(
				ctx, blks, cs, xs1, ys1, fl, stride, csc);
...
//			ct=ct0+(xs3*ys3)*4;
	...
	if((qf&BGBBTJ_BTIC1C_QFL_INDEXCLR) && (qf&BGBBTJ_QFL_IFRAME) &&
		((clrs==BGBBTJ_JPG_RGBA) || (clrs==BGBBTJ_JPG_RGB) ||
		 (clrs==BGBBTJ_JPG_BGRA) || (clrs==BGBBTJ_JPG_BGR)))
	{
		if(!hdr->pal_clr)
		{
			hdr->pal_nclr=256;
