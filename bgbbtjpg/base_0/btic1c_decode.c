...
//	}
	...
	{
		for(j=0; j<256; j++)
			{ hdr->pal256[j]=bt1c_blkidxcolor[j]; }
		for(j=0; j<16; j++)
			{ hdr->pal16[j]=bt1c_blkidxcolor[j]; }
		for(j=0; j<256; j++)
			{ hdr->pat256[j]=bt1c_blkidx_pixpat[j]; }
		for(j=0; j<8; j++)
			{ hdr->modes[j]=j; }
		for(j=0; j<8; j++)
			{ hdr->amodes[j]=j; }
...
//			hdr->sz_ldata=i;
	...
	case BGBBTJ_JPG_BGRA:
		...
		if(ctx->flip)
		{
			...
			BGBBTJ_BC7_DecodeImageBC7_BGRA(hdr->data, dst, xs, ys, 4);
		}
		else
		{
			i=BGBBTJ_BT1C_DecodeBlocksBC7(hdr, src, ssz,
			...
			BGBBTJ_BC7_DecodeImageBC7_BGRA(hdr->ldata, dst, xs, ys, 4);
		}
		bsz=16;
#endif
