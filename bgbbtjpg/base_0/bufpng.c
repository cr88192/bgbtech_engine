...
BGBBTJ_API int BGBBTJ_BufPNG_DecodeOBuf(
...
BGBBTJ_API byte *BGBBTJ_BufPNG_Decode(
...
//	vfwrite(buf, 1, len, fd);
	...
	*rct=(*rct)+len;
	...
	crc=BGBBTJ_BufPNG_CRC32(tb, 4, (uint)-1);
...
BGBBTJ_API int BGBBTJ_BufPNG_Encode(
...
BGBBTJ_API int BGBBTJ_BufPNG_EncodeFast(
	byte *ctbuf, int ctsz, byte *buf, int xs, int ys)
{
	byte hbuf[64];
	byte *buf2, *s, *t, *ct, *se;
