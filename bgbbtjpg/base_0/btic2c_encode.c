...
//	printf("M1\n");
	...
	{
#if 0
		BTIC2C_FilterImageDCT(ctx, ctx->yb, ctx->ydb, xs2, ys2);
		BTIC2C_FilterImageDCT(ctx, ctx->ub, ctx->udb, xs3, ys3);
		BTIC2C_FilterImageDCT(ctx, ctx->vb, ctx->vdb, xs3, ys3);
		if(ctx->img_alpha)
			{ BTIC2C_FilterImageDCT(ctx, ctx->ab, ctx->adb, xs3, ys3); }
#endif
	...
	{
#if 1
		BTIC2C_FilterImageDeltaWHT(ctx, ctx->yb, ctx->lyb,
			ctx->ydb, ctx->ybm, 0, xs2, ys2);
		BTIC2C_FilterImageDeltaWHT(ctx, ctx->ub, ctx->lub,
			ctx->udb, ctx->ubm, 1, xs3, ys3);
		BTIC2C_FilterImageDeltaWHT(ctx, ctx->vb, ctx->lvb,
			ctx->vdb, ctx->vbm, 1, xs3, ys3);
		if(ctx->img_alpha)
		{
			BTIC2C_FilterImageDeltaWHT(ctx, ctx->ab, ctx->lab,
